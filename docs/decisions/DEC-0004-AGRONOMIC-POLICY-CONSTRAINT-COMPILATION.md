...
The first real KBS benchmark exposed a predecessor schema limitation in accepted `adr.policy.v2`: it required non-empty `requiredRuntimeOutputs` even when the governed Policy was context-only.

That predecessor gap is now resolved by accepted DEC-0005 / `adr.policy.v3`. Policy v3 preserves v2 governed action semantics while allowing an explicit `requiredRuntimeOutputs: []`. The KBS prohibitions on Treatment 6, Treatment 7, Treatment 8nt and the 2,4-D tank mix can therefore remain source-bounded by ContextManifest/configuration or derived temporal context without inventing a RuntimeDatum.

ADR still must not relabel static context as a runtime output. The real-source KBS benchmark now proves honest operational constraint publication against exact Policy v3 authorities with zero required runtime-output ports.
...
Publication of a real KBS `AgronomicPolicyConstraintCompilation` requires a compatible exact Policy.

With accepted DEC-0005 / Policy v3, the current benchmark must prove:

```text
source-faithful claims = 5
qualified knowledge authorities = 5
real constraint compilations = 5
policy contract = adr.policy.v3
required runtime outputs = 0
execution authority records = 0
```

A synthetic authority fixture remains useful for isolated exact-authority and negative-case testing, but the real KBS source benchmark now also proves the full source-to-constraint publication path.
...
## Resolved predecessor dependency

The real KBS benchmark proved that negative agronomic authority can be decision-material without requiring any runtime model output.

DEC-0005 is now accepted and introduces `adr.policy.v3`, preserving Policy v1/v2 replay while allowing an explicit empty `requiredRuntimeOutputs` list for context-only governed Policies.

Using a ContextDatum/configuration semantic as a fake runtime output remains rejected.
