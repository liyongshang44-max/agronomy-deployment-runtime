...
ADR owns the semantics and governance of:

- Source and exact SourceArtifact provenance;
- Claim and SourceContext;
- scientific qualification;
- QualifiedKnowledge and DerivedKnowledge, including DerivedKnowledgeContext;
- KnowledgeConflict and KnowledgeRelease;
- Transformation, Model, Policy and Implementation specifications as distinct authorities;
- ImplementationConformance;
- scoped CalibrationArtifact authority;
- RuntimeProfile and Deployment;
- DecisionProblem;
- Agronomic Context Contract and ContextManifest;
- Source→Target transport/applicability;
- KnowledgeRetrievalResult provenance;
- RuntimePlan compilation and InformationRequirement;
- RuntimeEligibility;
- RuntimeBinding and RuntimeAlternativeSet;
- RuntimeResult/RuntimeDatum normalization;
- DecisionRobustness and DecisionResult when the decision authority mode permits;
- OutcomeEvaluation, explicit EffectAttributionAssessment where causality is claimed, and revision proposals;
...
```text
Scientific Compiler:
What does the exact source artifact actually say?

Qualification:
What may be recognized as usable knowledge?

Transport / Applicability:
Does it survive KnowledgeOriginContext → TargetContext for this purpose?

Runtime Compiler:
What exact computational world is legal here?

Decision Robustness:
Across the declared decision-material legal worlds, does the material action change?

Evaluation:
What did reality teach us after deployment, and what can actually be attributed causally?
```
...
- Model/policy/transformation implementations require explicit conformance authority.
...
`ADR` remains the product abbreviation and public contract namespace. Architecture/product decision records use the `DEC-xxxx` namespace.