...
export const KBS_T1R1_TARGET_OWNERSHIP = Object.freeze({ organizationId: 'org-a', tenantId: 'tenant-a' });
export const KBS_T1R1_PROVIDER_ID = 'kbs-public-context-adapter-v1';
export const KBS_T1R1_PROVIDER_HASH = 'sha256:b79542a4a5f19f91d7d98c6404bf841cd5660b28672e9fb66bd4b9e6905c1fcd';
export const KBS_T1R1_ADAPTER_LOCATOR =
  `urn:adr:acceptance:kbs-public-context-adapter:v1:mcse-t1r1-2026:${KBS_T1R1_PROVIDER_HASH}`;
export const KBS_T1R1_AVAILABLE_AT = '2026-09-05T03:25:00.000Z';
export const KBS_T1R1_RESOLVED_AT = '2026-09-05T03:30:00.000Z';
export const KBS_T1R1_EVIDENCE_CUTOFF = '2026-09-05T03:35:00.000Z';
export const KBS_T1R1_LOGICAL_TIME = '2026-09-05T03:40:00.000Z';
export const KBS_T1R1_DEADLINE = '2026-09-05T04:40:00.000Z';
export const KBS_T1R1_DECISION_TYPE = 'TARGET_CORRESPONDENCE_QUALIFICATION';

export function buildKbsT1R1TargetWorld() {
  const OWNERSHIP = KBS_T1R1_TARGET_OWNERSHIP;
  const providerBytes = readFileSync(new URL('./kbs-t1r1-2026-context-adapter-response.json', import.meta.url));
  const providerHash = providerResponseContentHash(providerBytes);
  assert.equal(providerHash, KBS_T1R1_PROVIDER_HASH);
  ...
  assert.equal(adapterResponse.world, 'KBS_MCSE_T1R1_2026_TARGET_CORRESPONDENCE');
  assert.equal(adapterResponse.retained_at, KBS_T1R1_AVAILABLE_AT);
  assert.deepEqual(adapterResponse.correspondence_basis.provider_target_components, [
    'Main Cropping System Experiment (MCSE)', 'T1', 'R1'
  ]);
  assert.equal(adapterResponse.source_evidence[0].treatment_code, 'T1');
  assert.equal(adapterResponse.source_evidence[0].replicate_structure, 'SIX_REPLICATE_BLOCKS');
  assert.equal(adapterResponse.source_evidence[1].observation_id, '6931');
  assert.equal(adapterResponse.source_evidence[1].replication_scope, 'ALL_T1_REPLICATIONS');
  ...
  assert.equal(adapterResponse.source_evidence[1].event_time_precision, 'LOCAL_CALENDAR_DAY_ONLY');
  assert.ok(adapterResponse.source_evidence[2].replicate_blocks.includes('R1'));
  assert.equal(adapterResponse.source_evidence[2].treatment_code, 'T1');
  ...
  assert.equal(adapterResponse.correspondence_basis.provider_geometry_republished, false);

  const ledger = new AuthorityLedger();
  const snapshotStore = new ExactContextSnapshotStore();
  const principal = Object.freeze({
    principalId: 'kbs-t1r1-context-gateway',
  ...
  function audit(suffix, occurredAt, inputRefs = []) {
    seq += 1;
    return {
      eventId: `evt-kbs-t1r1-target-${seq}-${suffix}`,
      occurredAt,
      actor: { type: principal.type, id: principal.principalId },
      inputRefs,
      details: {
        suite: 'real-kbs-t1r1-target-correspondence',
  ...
  function publishContextWriteAuthorization(logicalId, resourceType, occurredAt) {
    const role = publishRoleAssignment({
      ledger,
      logicalId: `role.kbs-t1r1.${resourceType.toLowerCase()}.${logicalId}`,
      version: '1',
      principal,
      role: 'KBS_T1R1_CONTEXT_GATEWAY',
      roleDefinitionVersion: 'kbs-t1r1-target-v1',
  ...
  for (const [semanticId, value] of semanticEntries) {
    ...
    const referenceId = `context-reference.kbs-t1r1-2026.${safe}`;
    const datumId = `context-datum.kbs-t1r1-2026.${safe}`;
    const receiptId = `context-receipt.kbs-t1r1-2026.${safe}`;

    const referenceAuth = publishContextWriteAuthorization(
      referenceId, 'AUTHORIZED_CONTEXT_REFERENCE', '2026-09-05T03:26:00.000Z'
    );
    const reference = publishAuthorizedContextReference({
      ...
      reference: {
        ...
        reference: {
          providerId: KBS_T1R1_PROVIDER_ID,
          locator: KBS_T1R1_ADAPTER_LOCATOR,
          addressingMode: 'CONTENT_ADDRESSED',
          expectedContentHash: providerHash
        },
        authorizationContext: {
          connectionId: KBS_T1R1_PROVIDER_ID,
          principalScope: {
            ...OWNERSHIP,
            subjectId: 'KBS_MCSE_T1_R1_2026',
      ...
      audit: audit(`reference-${safe}`, '2026-09-05T03:27:00.000Z')
    });
    ...
    const datumAuth = publishContextWriteAuthorization(
      datumId, 'CONTEXT_DATUM', '2026-09-05T03:28:00.000Z'
    );
    const datum = publishContextDatum({
      ...
      datum: {
        ...
        availableAt: KBS_T1R1_AVAILABLE_AT,
        ...
        source: {
          providerId: KBS_T1R1_PROVIDER_ID,
          sourceRef: `${KBS_T1R1_ADAPTER_LOCATOR}#${semanticId}`,
          contentHash: providerHash
        }
      },
      principal,
      authorizationDecisionAuditRef: datumAuth.ref,
      audit: audit(`datum-${safe}`, '2026-09-05T03:29:00.000Z')
    });
    ...
    const receiptAuth = publishContextWriteAuthorization(
      receiptId, 'RESOLVED_CONTEXT_DATUM_RECEIPT', '2026-09-05T03:29:30.000Z'
    );
    const receipt = publishResolvedContextDatumReceipt({
      ...
      resolution: {
        resolvedAt: KBS_T1R1_RESOLVED_AT,
      ...
      audit: audit(`receipt-${safe}`, KBS_T1R1_RESOLVED_AT)
    });
  ...
  const decisionLogicalId = 'decision-problem.kbs-t1r1-2026.target-correspondence-qualification';
  const decisionRole = publishRoleAssignment({
    ledger,
    logicalId: `role.kbs-t1r1.decision.${decisionLogicalId}`,
    version: '1',
    principal,
    role: 'KBS_T1R1_DECISION_CREATOR',
    roleDefinitionVersion: 'kbs-t1r1-target-v1',
    permissions: [PERMISSIONS.DECISION_PROBLEM_CREATE],
    scope: { ...OWNERSHIP, resourceType: 'DECISION_PROBLEM' },
    audit: audit('decision-role', '2026-09-05T03:31:00.000Z')
  });
  ...
  const decisionAuth = recordAuthorizationDecision({
    ...
    audit: audit('decision-auth', '2026-09-05T03:32:00.000Z')
  });
  const decision = publishDecisionProblem({
    ...
    problem: {
      ...
      decisionType: KBS_T1R1_DECISION_TYPE,
      targetRef: OWNERSHIP,
      logicalTime: KBS_T1R1_LOGICAL_TIME,
      ...
      decisionDeadline: KBS_T1R1_DEADLINE
    },
    principal,
    authorizationDecisionAuditRef: decisionAuth.ref,
    audit: audit('decision-publish', '2026-09-05T03:33:00.000Z')
  });
  ...
  assert.equal(validatedDecision.semanticPayload.decisionType, KBS_T1R1_DECISION_TYPE);
  ...
  const manifestLogicalId = 'context-manifest.kbs-t1r1-2026.target-correspondence-qualification';
  const manifestAuth = publishContextWriteAuthorization(
    manifestLogicalId, 'CONTEXT_MANIFEST', '2026-09-05T03:34:00.000Z'
  );
  const manifest = publishContextManifest({
    ...
    version: '1',
    decisionProblemRef: decision.ref,
    evidenceCutoff: KBS_T1R1_EVIDENCE_CUTOFF,
    datumRefs,
    resolvedReferenceReceiptRefs: receiptRefs,
    snapshotStore,
    principal,
    authorizationDecisionAuditRef: manifestAuth.ref,
    audit: audit('manifest-publish', '2026-09-05T03:36:00.000Z')
  });
  ...
  assert.deepEqual(contextBySemanticId['treatment.code'], { type: 'CATEGORY', category: 'T1' });
  ...
  assert.deepEqual(contextBySemanticId['planting.observation_id'], { type: 'STRING', string: '6931' });
