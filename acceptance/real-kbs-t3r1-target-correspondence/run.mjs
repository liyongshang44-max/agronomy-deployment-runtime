...
assert.equal(world.providerHash, 'sha256:e21b6d9412cfe62600bb92a93a9359a9c08f7595451f62a4a505af43d27a58c5');
...
const geoxTargetAuthority = JSON.parse(readFileSync(
  new URL('./geox-kbs-t3r1-authority-export.json', import.meta.url),
  'utf8'
));
assert.equal(geoxTargetAuthority.source_main_sha, '5050f1c08d2528048c56d56add4cbb068b956925');
assert.equal(geoxTargetAuthority.geox_target.field_id, 'field_kbs_mcse_t3r1');
...
const message = createIntegrationMessage({
  ...
  messageId: 'geox-kbs-t3r1-target-correspondence-1',
  authorityRefs,
  payload: {
    provider_target: {
...
assert.equal(projection.relation, GEOX_TARGET_CORRESPONDENCE_T3R1_RELATION);
assert.equal(
...
assert.equal(projection.provider_target.treatment_code, 'T3');
...
assert.equal(projection.provider_target.planting_observation_id, '6966');
assert.equal(projection.geox_target.field_id, 'field_kbs_mcse_t3r1');
assert.equal(projection.geox_target.zone_id, 'zone_kbs_mcse_t3r1_crop_formal_v1');
...
console.log(JSON.stringify({
  ok: true,
  milestone: 'PRODUCTIZATION_KBS_T3R1_CROSS_NAMESPACE_TARGET_CORRESPONDENCE_V1',
  adrTarget: {
