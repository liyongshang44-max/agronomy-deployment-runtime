...
function expectUnitRepresentationError(fn, code) {
  assert.throws(fn, (error) => {
    assert.ok(
      error instanceof AgronomicContextNonQuantitativeUnitRepresentationCompilationError,
      `expected DEC-0024 typed error, got ${error?.constructor?.name ?? 'none'}`
    );
...
function buildUnitRepresentation(
  ...
  return {
    contractVersion:
      AGRONOMIC_CONTEXT_NON_QUANTITATIVE_UNIT_REPRESENTATION_CONTRACT_VERSION,
    representationId:
      'representation.gold.sustainable-corn.planting-date.not-applicable-unit',
    ...
    rationale:
      'The exact DEC-0016 crop.planting_date DATE value is non-quantitative; no measurement unit applies, so the frozen required unit field uses the reviewed canonical NOT_APPLICABLE token.',
    ...overrides
  };
}

function buildUnitRepresentationCompilation(
  representation,
  reviewRef,
  status = 'COMPLETE'
) {
  return {
    ...
    losslessCoverage: {
      ...
      unrepresentedElements:
        status === 'COMPLETE'
          ? []
          : ['UNRESOLVED_UNIT_REPRESENTATION_ELEMENT']
    },
    limitations: [
      'UNIT_REPRESENTATION_NOT_CONTEXT_DATUM',
      'NOT_APPLICABLE_NOT_DIMENSIONLESS_QUANTITY',
      'NO_GENERIC_DATE_TYPE_RULE',
      'NO_UNCERTAINTY_AUTHORITY',
      'NO_EFFECTIVE_INTERVAL_AUTHORITY',
      'NO_AVAILABLE_AT_AUTHORITY',
      'NO_TEMPORAL_TIMEZONE_SPATIAL_MUTATION',
      'NO_TARGET_INSTANCE_OR_GEOMETRY_AUTHORITY',
      'NO_EPISTEMIC_PROVENANCE_SOURCE_MUTATION',
      'NO_GENERAL_UNIT_REGISTRY_OR_CONVERSION_AUTHORITY',
      'NO_MISSING_DATA_SUBSTITUTION'
    ]
  };
}

const unitRepresentation = buildUnitRepresentation();
const unitRepresentationReview =
  publishAgronomicContextNonQuantitativeUnitRepresentationReviewDecision({
    ledger: env.ledger,
    sourceRegistry: env.sourceRegistry,
    logicalId:
      'review.gold.sustainable-corn.context-non-quantitative-unit-representation',
    version: '1',
    ...
    confirmedChecks:
      AGRONOMIC_CONTEXT_NON_QUANTITATIVE_UNIT_REPRESENTATION_REQUIRED_REVIEW_CHECKS,
    rationale:
      'Exact revalidated DEC-0016 planting-date semantic/value admits only canonical NOT_APPLICABLE; day, unitless, missing-data and generic DATE inference are explicitly rejected.',
    audit: audit(
      'evt-gold-context-non-quantitative-unit-representation-review',
      normalizationReviewer.principalId
    )
  });

const unitRepresentationPublished =
  publishAgronomicContextNonQuantitativeUnitRepresentationCompilation({
    ledger: env.ledger,
    sourceRegistry: env.sourceRegistry,
    logicalId:
      'compilation.gold.sustainable-corn.context-non-quantitative-unit-representation',
    version: '1',
    compilation: buildUnitRepresentationCompilation(
      unitRepresentation,
      unitRepresentationReview.ref
    ),
    audit: audit(
      'evt-gold-context-non-quantitative-unit-representation-publication',
      normalizationReviewer.principalId
    )
  });

const unitRepresentationValidated =
  validateAgronomicContextNonQuantitativeUnitRepresentationCompilationAuthority({
    ledger: env.ledger,
    sourceRegistry: env.sourceRegistry,
    compilationRef: unitRepresentationPublished.ref
  });

assert.deepEqual(
  unitRepresentationValidated.semanticPayload.representation.targetContextSemantic,
  {
    semanticId: 'crop.planting_date',
    value: { type: 'DATE', date: '2011-05-03' }
  }
...
assert.deepEqual(
  unitRepresentationValidated.semanticPayload.representation.unitRepresentation,
  { kind: 'NOT_APPLICABLE', wireValue: 'NOT_APPLICABLE' }
);
assert.deepEqual(
  unitRepresentationValidated.contextSemanticMapping.semanticPayload.mapping
    .targetContextSemantic,
  unitRepresentationValidated.semanticPayload.representation.targetContextSemantic
);

expectUnitRepresentationError(
  () =>
    publishAgronomicContextNonQuantitativeUnitRepresentationReviewDecision({
      ledger: env.ledger,
      sourceRegistry: env.sourceRegistry,
      logicalId: 'review.gold.dec0024.wrong-parent-kind',
      version: '1',
      representation: buildUnitRepresentation(normalizationPublished.ref),
      disposition: 'ACCEPT_CONTEXT_NON_QUANTITATIVE_UNIT_REPRESENTATION',
      reviewerPrincipal: normalizationReviewer,
      authorizationDecisionAuditRefs: [
        parentSourceAuthorization.auth.ref,
      ...
      audit: audit(
        'evt-gold-dec0024-wrong-parent-kind',
        normalizationReviewer.principalId
      )
    }),
  ...
  expectUnitRepresentationError(
    () =>
      publishAgronomicContextNonQuantitativeUnitRepresentationReviewDecision({
        ledger: env.ledger,
        sourceRegistry: env.sourceRegistry,
        logicalId: `review.gold.dec0024.${label}`,
        version: '1',
        representation: buildUnitRepresentation(mappingPublished.ref, {
          targetContextSemantic
        }),
        disposition: 'ACCEPT_CONTEXT_NON_QUANTITATIVE_UNIT_REPRESENTATION',
        reviewerPrincipal: normalizationReviewer,
...
expectUnitRepresentationError(
  () =>
    publishAgronomicContextNonQuantitativeUnitRepresentationReviewDecision({
      ledger: env.ledger,
      sourceRegistry: env.sourceRegistry,
      logicalId: 'review.gold.dec0024.missing-data-substitution',
      version: '1',
      representation: {
        ...buildUnitRepresentation(),
      ...
      confirmedChecks:
        AGRONOMIC_CONTEXT_NON_QUANTITATIVE_UNIT_REPRESENTATION_REQUIRED_REVIEW_CHECKS,
      rationale: 'NOT_APPLICABLE cannot encode missing or not-reported unit state.',
      audit: audit(
        'evt-gold-dec0024-missing-data-substitution',
        normalizationReviewer.principalId
      )
    }),
...
assert.throws(() =>
  publishAgronomicContextNonQuantitativeUnitRepresentationReviewDecision({
    ledger: env.ledger,
    sourceRegistry: env.sourceRegistry,
    logicalId: 'review.gold.dec0024.predecessor-ref-drift',
    version: '1',
    representation: buildUnitRepresentation(predecessorRefDrift),
    disposition: 'ACCEPT_CONTEXT_NON_QUANTITATIVE_UNIT_REPRESENTATION',
    reviewerPrincipal: normalizationReviewer,
    authorizationDecisionAuditRefs: [
      parentSourceAuthorization.auth.ref,
    ...
    confirmedChecks:
      AGRONOMIC_CONTEXT_NON_QUANTITATIVE_UNIT_REPRESENTATION_REQUIRED_REVIEW_CHECKS,
    rationale: 'Exact predecessor ref drift must fail closed during DEC-0016 revalidation.',
    audit: audit(
      'evt-gold-dec0024-predecessor-ref-drift',
      normalizationReviewer.principalId
    )
  })
);
...
expectUnitRepresentationError(
  () =>
    publishAgronomicContextNonQuantitativeUnitRepresentationReviewDecision({
      ledger: env.ledger,
      sourceRegistry: env.sourceRegistry,
      logicalId: 'review.gold.dec0024.incomplete-review',
      version: '1',
      representation: buildUnitRepresentation(),
      disposition: 'ACCEPT_CONTEXT_NON_QUANTITATIVE_UNIT_REPRESENTATION',
      reviewerPrincipal: normalizationReviewer,
      authorizationDecisionAuditRefs: [
        parentSourceAuthorization.auth.ref,
      ...
      rationale: 'Incomplete review cannot authorize unit representation.',
      audit: audit(
        'evt-gold-dec0024-incomplete-review',
        normalizationReviewer.principalId
      )
    }),
...
const unauthorizedUnitReviewer = createPrincipal({
  principalId: 'gold-dec0024-unauthorized-reviewer',
  type: 'USER',
  organizationId: 'org-a',
  tenantId: 'tenant-a'
});
expectUnitRepresentationError(
  () =>
    publishAgronomicContextNonQuantitativeUnitRepresentationReviewDecision({
      ledger: env.ledger,
      sourceRegistry: env.sourceRegistry,
      logicalId: 'review.gold.dec0024.unauthorized-reviewer',
      version: '1',
      representation: buildUnitRepresentation(),
      disposition: 'ACCEPT_CONTEXT_NON_QUANTITATIVE_UNIT_REPRESENTATION',
      reviewerPrincipal: unauthorizedUnitReviewer,
      authorizationDecisionAuditRefs: [
        parentSourceAuthorization.auth.ref,
        semanticSourceAuthorization.auth.ref
      ],
      confirmedChecks:
        AGRONOMIC_CONTEXT_NON_QUANTITATIVE_UNIT_REPRESENTATION_REQUIRED_REVIEW_CHECKS,
      rationale: 'Reviewer cannot borrow another reviewer source authorizations.',
      audit: audit(
        'evt-gold-dec0024-unauthorized-reviewer',
        unauthorizedUnitReviewer.principalId
      )
    }),
  'AGRONOMIC_CONTEXT_NON_QUANTITATIVE_UNIT_REPRESENTATION_REVIEW_AUTHORIZATION_INVALID'
);

const rejectedUnitReview =
  publishAgronomicContextNonQuantitativeUnitRepresentationReviewDecision({
    ledger: env.ledger,
    sourceRegistry: env.sourceRegistry,
    logicalId: 'review.gold.dec0024.rejected',
    version: '1',
    representation: buildUnitRepresentation(),
    disposition: 'REJECT_CONTEXT_NON_QUANTITATIVE_UNIT_REPRESENTATION',
    reviewerPrincipal: normalizationReviewer,
    authorizationDecisionAuditRefs: [
      parentSourceAuthorization.auth.ref,
      semanticSourceAuthorization.auth.ref
    ...
    audit: audit(
      'evt-gold-dec0024-rejected-review',
      normalizationReviewer.principalId
    )
  });
expectUnitRepresentationError(
  () =>
    publishAgronomicContextNonQuantitativeUnitRepresentationCompilation({
      ledger: env.ledger,
      sourceRegistry: env.sourceRegistry,
      logicalId: 'compilation.gold.dec0024.rejected',
      version: '1',
      compilation: buildUnitRepresentationCompilation(
        buildUnitRepresentation(),
        rejectedUnitReview.ref
      ),
      audit: audit(
        'evt-gold-dec0024-rejected-publication',
        normalizationReviewer.principalId
      )
    }),
...
const reviewMismatchRepresentation = buildUnitRepresentation(
...
expectUnitRepresentationError(
  () =>
    publishAgronomicContextNonQuantitativeUnitRepresentationCompilation({
      ledger: env.ledger,
      sourceRegistry: env.sourceRegistry,
      logicalId: 'compilation.gold.dec0024.review-mismatch',
      version: '1',
      compilation: buildUnitRepresentationCompilation(
      ...
      audit: audit(
        'evt-gold-dec0024-review-mismatch-publication',
        normalizationReviewer.principalId
      )
    }),
...
const forbiddenAfterUnitRepresentation = env.ledger.exportSnapshot().records
  .filter((record) => forbiddenKinds.has(record.ref.kind));
assert.equal(forbiddenAfterUnitRepresentation.length, 0);

console.log(JSON.stringify({
  ok: true,
  authority: 'AgronomicContextNonQuantitativeUnitRepresentationCompilation',
  ...
  negativeCases: [
    ...
    'DAY_DURATION_SUBSTITUTION_DENIED',
    'UNITLESS_SUBSTITUTION_DENIED',
    'EMPTY_NULL_OMITTED_UNIT_DENIED',
  ...
  forbiddenDownstreamAuthorityRecordsCreated:
    forbiddenAfterUnitRepresentation.length
}, null, 2));
