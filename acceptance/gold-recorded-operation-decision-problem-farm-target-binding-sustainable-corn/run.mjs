...
function expectSpatialSupportClassificationError(fn, code) {
  let caught;
  try {
    fn();
  } catch (error) {
  ...
  assert.ok(
    caught instanceof
      AgronomicRecordedOperationContextSpatialSupportClassificationCompilationError,
    `expected spatial-support classification error, got ${caught?.constructor?.name ?? 'none'}`
  );
  assert.equal(caught.code, code);
}

function buildSpatialSupportClassification({
  ...
  return {
    contractVersion:
      AGRONOMIC_RECORDED_OPERATION_CONTEXT_SPATIAL_SUPPORT_CLASSIFICATION_CONTRACT_VERSION,
    classificationId:
      'sustainable-corn.serf.crop-planting-date.spatial-support',
    contextSemanticMappingCompilationRef,
    targetIdentityBindingCompilationRef,
    ...
    classificationRationale:
      'Classify the exact DEC-0015 source-backed FARM target granularity as Context spatial support type FARM for the exact DEC-0016 planting-date semantic; retain exact target identity only in governed lineage and create no geometryRef or target-instance field.'
  };
}

function buildSpatialSupportClassificationCompilation(
  classification,
  reviewRef
) {
  return {
    ...
    limitations: [
      ...
      'NO_FIELD_PLOT_ZONE_REFINEMENT',
      'NO_UNIFORM_WITHIN_FARM_COVERAGE',
      'SUPPORT_TYPE_NOT_TARGET_INSTANCE_IDENTITY',
      'NO_DECISION_PROBLEM_TARGET_PROJECTION',
      'NO_TEMPORAL_FRAME_OR_EFFECTIVE_TIME',
      'NO_UNIT_UNCERTAINTY_VERTICAL_SUPPORT',
      'NO_CONTEXT_DATUM_PUBLICATION'
    ]
  };
}
...
const spatialClassification =
  buildSpatialSupportClassification({
    contextSemanticMappingCompilationRef: mappingPublished.ref,
...
const spatialReview =
  publishAgronomicRecordedOperationContextSpatialSupportClassificationReviewDecision({
    ...
    sourceRegistry: env.sourceRegistry,
    logicalId:
      'review.gold.sustainable-corn.context-spatial-support-classification',
    version: '1',
    ...
    confirmedChecks:
      AGRONOMIC_RECORDED_OPERATION_CONTEXT_SPATIAL_SUPPORT_CLASSIFICATION_REQUIRED_REVIEW_CHECKS,
    rationale:
      'Exact DEC-0016 and DEC-0015 authorities converge on the same DEC-0013 SERF occurrence; classify only FARM support granularity and retain exact target identity as lineage, not geometry.',
    audit: audit(
      'evt-gold-dec0023-spatial-review',
      normalizationReviewer.principalId
    )
  });

const spatialPublished =
  publishAgronomicRecordedOperationContextSpatialSupportClassificationCompilation({
    ledger: env.ledger,
    sourceRegistry: env.sourceRegistry,
    logicalId:
      'compilation.gold.sustainable-corn.context-spatial-support-classification',
    version: '1',
    compilation:
      buildSpatialSupportClassificationCompilation(
        spatialClassification,
        spatialReview.ref
      ),
    audit: audit(
      'evt-gold-dec0023-spatial-publication',
      normalizationReviewer.principalId
    )
  });

const spatialValidated =
  validateAgronomicRecordedOperationContextSpatialSupportClassificationCompilationAuthority({
    ledger: env.ledger,
    sourceRegistry: env.sourceRegistry,
    compilationRef: spatialPublished.ref
  });

assert.deepEqual(
  spatialValidated.semanticPayload.classification.targetContextSemantic,
  {
    semanticId: 'crop.planting_date',
    value: { type: 'DATE', date: '2011-05-03' }
  }
...
assert.deepEqual(
  spatialValidated.semanticPayload.classification.sourceNativeSubject,
  { name: 'siteid', value: 'SERF' }
);
assert.equal(
...
assert.deepEqual(
  spatialValidated.parentOccurrence.record.ref,
  mappingValidated.parentOccurrence.record.ref
);
assert.deepEqual(
  spatialValidated.parentOccurrence.record.ref,
  targetIdentityValidated.parentOccurrence.record.ref
...
for (const type of [
  ...
  const drift = structuredClone(spatialClassification);
  drift.spatialSupport.type = type;
  expectSpatialSupportClassificationError(
    () =>
      publishAgronomicRecordedOperationContextSpatialSupportClassificationReviewDecision({
        ledger: env.ledger,
        sourceRegistry: env.sourceRegistry,
        logicalId: 'review.gold.dec0023.spatial-type-drift',
        version: '1',
        classification: drift,
        disposition:
          'ACCEPT_RECORDED_OPERATION_CONTEXT_SPATIAL_SUPPORT_CLASSIFICATION',
        ...
        confirmedChecks:
          AGRONOMIC_RECORDED_OPERATION_CONTEXT_SPATIAL_SUPPORT_CLASSIFICATION_REQUIRED_REVIEW_CHECKS,
        rationale: 'Only FARM support classification is accepted in v1.',
        audit: audit(
          'evt-gold-dec0023-spatial-type-drift',
          normalizationReviewer.principalId
        )
      }),
...
const geometryRefDrift = structuredClone(spatialClassification);
geometryRefDrift.spatialSupport.geometryRef =
  geometryRefDrift.sourceBackedTargetIdentity.targetId;
expectSpatialSupportClassificationError(
  () =>
    publishAgronomicRecordedOperationContextSpatialSupportClassificationReviewDecision({
      ledger: env.ledger,
      sourceRegistry: env.sourceRegistry,
      logicalId: 'review.gold.dec0023.geometry-ref-drift',
      version: '1',
      classification: geometryRefDrift,
      disposition:
        'ACCEPT_RECORDED_OPERATION_CONTEXT_SPATIAL_SUPPORT_CLASSIFICATION',
      reviewerPrincipal: normalizationReviewer,
      authorizationDecisionAuditRefs: [
      ...
      confirmedChecks:
        AGRONOMIC_RECORDED_OPERATION_CONTEXT_SPATIAL_SUPPORT_CLASSIFICATION_REQUIRED_REVIEW_CHECKS,
      rationale: 'Target identity cannot be laundered into geometryRef.',
      audit: audit(
        'evt-gold-dec0023-geometry-ref-drift',
        normalizationReviewer.principalId
      )
    }),
...
const targetIdDrift = structuredClone(spatialClassification);
targetIdDrift.sourceBackedTargetIdentity.targetId =
  'target_src_fabricated_not_dec0015';
expectSpatialSupportClassificationError(
  () =>
    publishAgronomicRecordedOperationContextSpatialSupportClassificationReviewDecision({
      ledger: env.ledger,
      sourceRegistry: env.sourceRegistry,
      logicalId: 'review.gold.dec0023.target-id-drift',
      version: '1',
      classification: targetIdDrift,
      disposition:
        'ACCEPT_RECORDED_OPERATION_CONTEXT_SPATIAL_SUPPORT_CLASSIFICATION',
      reviewerPrincipal: normalizationReviewer,
      authorizationDecisionAuditRefs: [
      ...
      confirmedChecks:
        AGRONOMIC_RECORDED_OPERATION_CONTEXT_SPATIAL_SUPPORT_CLASSIFICATION_REQUIRED_REVIEW_CHECKS,
      rationale: 'Copied or fabricated targetId must fail DEC-0015 lineage replay.',
      audit: audit(
        'evt-gold-dec0023-target-id-drift',
        normalizationReviewer.principalId
      )
    }),
...
const contextDrift = structuredClone(spatialClassification);
contextDrift.targetContextSemantic.value.date = '2011-05-04';
expectSpatialSupportClassificationError(
  () =>
    publishAgronomicRecordedOperationContextSpatialSupportClassificationReviewDecision({
      ledger: env.ledger,
      sourceRegistry: env.sourceRegistry,
      logicalId: 'review.gold.dec0023-context-drift',
      version: '1',
      classification: contextDrift,
      disposition:
        'ACCEPT_RECORDED_OPERATION_CONTEXT_SPATIAL_SUPPORT_CLASSIFICATION',
      reviewerPrincipal: normalizationReviewer,
      authorizationDecisionAuditRefs: [
      ...
      confirmedChecks:
        AGRONOMIC_RECORDED_OPERATION_CONTEXT_SPATIAL_SUPPORT_CLASSIFICATION_REQUIRED_REVIEW_CHECKS,
      rationale: 'Context semantic/value drift must fail closed.',
      audit: audit(
        'evt-gold-dec0023-context-drift',
        normalizationReviewer.principalId
      )
    }),
...
expectSpatialSupportClassificationError(
  () =>
    publishAgronomicRecordedOperationContextSpatialSupportClassificationReviewDecision({
      ledger: env.ledger,
      sourceRegistry: env.sourceRegistry,
      logicalId: 'review.gold.dec0023.incomplete-review',
      version: '1',
      classification: spatialClassification,
      disposition:
        'ACCEPT_RECORDED_OPERATION_CONTEXT_SPATIAL_SUPPORT_CLASSIFICATION',
      ...
      confirmedChecks:
        AGRONOMIC_RECORDED_OPERATION_CONTEXT_SPATIAL_SUPPORT_CLASSIFICATION_REQUIRED_REVIEW_CHECKS
          .filter((item) => item !== 'NO_GEOMETRY_REF'),
      rationale: 'Incomplete spatial review cannot authorize publication.',
      audit: audit(
        'evt-gold-dec0023-incomplete-review',
        normalizationReviewer.principalId
      )
    }),
...
const unauthorizedSpatialReviewer = createPrincipal({
  principalId: 'gold-dec0023-spatial-unauthorized-reviewer',
  type: 'USER',
  organizationId: 'org-a',
  tenantId: 'tenant-a'
});
expectSpatialSupportClassificationError(
  () =>
    publishAgronomicRecordedOperationContextSpatialSupportClassificationReviewDecision({
      ledger: env.ledger,
      sourceRegistry: env.sourceRegistry,
      logicalId: 'review.gold.dec0023.unauthorized',
      version: '1',
      classification: spatialClassification,
      disposition:
        'ACCEPT_RECORDED_OPERATION_CONTEXT_SPATIAL_SUPPORT_CLASSIFICATION',
      reviewerPrincipal: unauthorizedSpatialReviewer,
      authorizationDecisionAuditRefs: [
        parentSourceAuthorization.auth.ref,
        semanticSourceAuthorization.auth.ref,
        targetIdentityAuthorization.auth.ref
      ...
      audit: audit(
        'evt-gold-dec0023-unauthorized',
        unauthorizedSpatialReviewer.principalId
      )
    }),
  'AGRONOMIC_RECORDED_OPERATION_CONTEXT_SPATIAL_SUPPORT_CLASSIFICATION_REVIEW_AUTHORIZATION_INVALID'
);

const rejectedSpatialReview =
  publishAgronomicRecordedOperationContextSpatialSupportClassificationReviewDecision({
    ledger: env.ledger,
    sourceRegistry: env.sourceRegistry,
    logicalId: 'review.gold.dec0023.rejected',
    version: '1',
    classification: spatialClassification,
    disposition:
    ...
    confirmedChecks: [],
    rationale: 'Rejected spatial review cannot authorize publication.',
    audit: audit(
      'evt-gold-dec0023-rejected-review',
      normalizationReviewer.principalId
    )
  });
expectSpatialSupportClassificationError(
  () =>
    publishAgronomicRecordedOperationContextSpatialSupportClassificationCompilation({
      ledger: env.ledger,
      sourceRegistry: env.sourceRegistry,
      logicalId: 'compilation.gold.dec0023.rejected',
      version: '1',
      compilation:
        buildSpatialSupportClassificationCompilation(
          spatialClassification,
          rejectedSpatialReview.ref
        ),
      audit: audit(
        'evt-gold-dec0023-rejected-publication',
        normalizationReviewer.principalId
      )
    }),
...
const spatialCoPredecessorDrift = structuredClone(spatialClassification);
spatialCoPredecessorDrift.targetIdentityBindingCompilationRef =
...
expectSpatialSupportClassificationError(
  () =>
    publishAgronomicRecordedOperationContextSpatialSupportClassificationReviewDecision({
      ledger: env.ledger,
      sourceRegistry: env.sourceRegistry,
      logicalId: 'review.gold.dec0023.co-predecessor-drift',
      version: '1',
      classification: spatialCoPredecessorDrift,
      disposition:
        'ACCEPT_RECORDED_OPERATION_CONTEXT_SPATIAL_SUPPORT_CLASSIFICATION',
      reviewerPrincipal: normalizationReviewer,
      authorizationDecisionAuditRefs: [
      ...
      confirmedChecks:
        AGRONOMIC_RECORDED_OPERATION_CONTEXT_SPATIAL_SUPPORT_CLASSIFICATION_REQUIRED_REVIEW_CHECKS,
      rationale:
        'Semantically similar but authority-distinct DEC-0013 parents must fail exact co-predecessor convergence.',
      audit: audit(
        'evt-gold-dec0023-co-predecessor-drift',
        normalizationReviewer.principalId
      )
    }),
...
function expectTargetRefProjectionError(fn, code) {
  assert.throws(fn, (error) => {
    assert.ok(
      error instanceof AgronomicContextTargetRefFarmInstanceProjectionCompilationError,
      `expected DEC-0027 typed error, got ${error?.constructor?.name ?? 'none'}`
    );
...
function buildTargetRefFarmProjection(overrides = {}) {
  const identity =
    spatialValidated.targetIdentityBinding.semanticPayload.binding
      .sourceBackedTargetIdentity;
  return {
    contractVersion:
      AGRONOMIC_CONTEXT_TARGET_REF_FARM_INSTANCE_PROJECTION_CONTRACT_VERSION,
    projectionId:
      'projection.gold.sustainable-corn.target-ref-farm-instance',
    parentContextSpatialSupportClassificationCompilationRef: spatialPublished.ref,
    targetContextSemantic: {
      semanticId: 'crop.planting_date',
      value: { type: 'DATE', date: '2011-05-03' }
    },
    ...
    targetRefProjection: {
      field: 'farmId',
      value: identity.targetId
    },
    rationale:
      'Project the exact DEC-0015 source-backed FARM target identity revalidated through DEC-0023 into targetRef.farmId only; do not create a complete targetRef.',
    ...overrides
  };
}

function buildTargetRefFarmProjectionCompilation(
  projection,
  reviewRef,
  status = 'COMPLETE'
) {
  return {
    ...
    losslessCoverage: {
      ...
      unrepresentedElements:
        status === 'COMPLETE' ? [] : ['UNRESOLVED_TARGET_REF_PROJECTION_ELEMENT']
    },
    limitations: [
      ...
      'NO_TEMPORAL_OR_AVAILABLE_AT_AUTHORITY',
      'NO_CROSS_PROVIDER_IDENTITY_EQUIVALENCE'
    ]
  };
}

const targetRefProjection = buildTargetRefFarmProjection();
const targetRefReview =
  publishAgronomicContextTargetRefFarmInstanceProjectionReviewDecision({
    ledger: env.ledger,
    sourceRegistry: env.sourceRegistry,
    logicalId:
      'review.gold.sustainable-corn.context-target-ref-farm-instance-projection',
    version: '1',
    ...
    confirmedChecks:
      AGRONOMIC_CONTEXT_TARGET_REF_FARM_INSTANCE_PROJECTION_REQUIRED_REVIEW_CHECKS,
    rationale:
      'Exact DEC-0023 revalidation closes through exact DEC-0015 source-backed FARM identity; project only the exact target_src targetId into targetRef.farmId.',
    audit: audit(
      'evt-gold-dec0027-target-ref-farm-review',
      normalizationReviewer.principalId
    )
  });

const targetRefPublished =
  publishAgronomicContextTargetRefFarmInstanceProjectionCompilation({
    ledger: env.ledger,
    sourceRegistry: env.sourceRegistry,
    logicalId:
      'compilation.gold.sustainable-corn.context-target-ref-farm-instance-projection',
    version: '1',
    compilation: buildTargetRefFarmProjectionCompilation(
      targetRefProjection,
      targetRefReview.ref
    ),
    audit: audit(
      'evt-gold-dec0027-target-ref-farm-publication',
      normalizationReviewer.principalId
    )
  });

const targetRefValidated =
  validateAgronomicContextTargetRefFarmInstanceProjectionCompilationAuthority({
    ledger: env.ledger,
    sourceRegistry: env.sourceRegistry,
    compilationRef: targetRefPublished.ref
  });

const exactTargetId =
  spatialValidated.targetIdentityBinding.semanticPayload.binding
    .sourceBackedTargetIdentity.targetId;
assert.match(exactTargetId, /^target_src_[0-9a-f]{64}$/);
assert.deepEqual(
  targetRefValidated.semanticPayload.projection.targetContextSemantic,
  {
    semanticId: 'crop.planting_date',
    value: { type: 'DATE', date: '2011-05-03' }
  }
...
assert.equal(
  targetRefValidated.semanticPayload.projection.sourceBackedTargetIdentity
    .granularity,
  'FARM'
);
assert.equal(
...
assert.deepEqual(
  targetRefValidated.semanticPayload.projection.targetRefProjection,
  { field: 'farmId', value: exactTargetId }
);
assert.notEqual(
  targetRefValidated.semanticPayload.projection.targetRefProjection.value,
  'SERF'
);
assert.notEqual(
  targetRefValidated.semanticPayload.projection.targetRefProjection.value,
...
expectTargetRefProjectionError(
  () =>
    publishAgronomicContextTargetRefFarmInstanceProjectionReviewDecision({
      ledger: env.ledger,
      sourceRegistry: env.sourceRegistry,
      logicalId: 'review.gold.dec0027.wrong-parent-kind',
      version: '1',
      projection: buildTargetRefFarmProjection({
        parentContextSpatialSupportClassificationCompilationRef: mappingPublished.ref
      }),
      disposition: 'ACCEPT_CONTEXT_TARGET_REF_FARM_INSTANCE_PROJECTION',
      reviewerPrincipal: normalizationReviewer,
      authorizationDecisionAuditRefs: [
      ...
      audit: audit(
        'evt-gold-dec0027-wrong-parent-kind',
        normalizationReviewer.principalId
      )
    }),
...
const predecessorRefDrift = structuredClone(spatialPublished.ref);
predecessorRefDrift.semanticHash = `sha256:${'0'.repeat(64)}`;
assert.throws(() =>
  publishAgronomicContextTargetRefFarmInstanceProjectionReviewDecision({
    ledger: env.ledger,
    sourceRegistry: env.sourceRegistry,
    logicalId: 'review.gold.dec0027.predecessor-ref-drift',
    version: '1',
    projection: buildTargetRefFarmProjection({
      parentContextSpatialSupportClassificationCompilationRef: predecessorRefDrift
    }),
    disposition: 'ACCEPT_CONTEXT_TARGET_REF_FARM_INSTANCE_PROJECTION',
    reviewerPrincipal: normalizationReviewer,
    authorizationDecisionAuditRefs: [
    ...
    audit: audit(
      'evt-gold-dec0027-predecessor-ref-drift',
      normalizationReviewer.principalId
    )
  })
);
  ...
  expectTargetRefProjectionError(
    () =>
      publishAgronomicContextTargetRefFarmInstanceProjectionReviewDecision({
        ledger: env.ledger,
        sourceRegistry: env.sourceRegistry,
        logicalId: `review.gold.dec0027.${label}`,
        version: '1',
        projection: buildTargetRefFarmProjection({ targetContextSemantic }),
        disposition: 'ACCEPT_CONTEXT_TARGET_REF_FARM_INSTANCE_PROJECTION',
        reviewerPrincipal: normalizationReviewer,
        authorizationDecisionAuditRefs: [
          parentSourceAuthorization.auth.ref,
...
expectTargetRefProjectionError(
  () =>
    publishAgronomicContextTargetRefFarmInstanceProjectionReviewDecision({
      ledger: env.ledger,
      sourceRegistry: env.sourceRegistry,
      logicalId: 'review.gold.dec0027.non-farm',
      version: '1',
      projection: buildTargetRefFarmProjection({
        sourceBackedTargetIdentity: {
          namespaceRef:
            targetRefProjection.sourceBackedTargetIdentity.namespaceRef,
          granularity: 'FIELD',
      ...
      confirmedChecks:
        AGRONOMIC_CONTEXT_TARGET_REF_FARM_INSTANCE_PROJECTION_REQUIRED_REVIEW_CHECKS,
      rationale: 'Finer target granularity must fail closed.',
      audit: audit(
        'evt-gold-dec0027-non-farm',
        normalizationReviewer.principalId
      )
    }),
...
const driftTargetId = `target_src_${'b'.repeat(64)}`;
expectTargetRefProjectionError(
  () =>
    publishAgronomicContextTargetRefFarmInstanceProjectionReviewDecision({
      ledger: env.ledger,
      sourceRegistry: env.sourceRegistry,
      logicalId: 'review.gold.dec0027.target-id-drift',
      version: '1',
      projection: buildTargetRefFarmProjection({
        sourceBackedTargetIdentity: {
          namespaceRef:
            targetRefProjection.sourceBackedTargetIdentity.namespaceRef,
          granularity: 'FARM',
          targetId: driftTargetId
        },
        targetRefProjection: { field: 'farmId', value: driftTargetId }
      }),
      disposition: 'ACCEPT_CONTEXT_TARGET_REF_FARM_INSTANCE_PROJECTION',
      reviewerPrincipal: normalizationReviewer,
      authorizationDecisionAuditRefs: [
      ...
      confirmedChecks:
        AGRONOMIC_CONTEXT_TARGET_REF_FARM_INSTANCE_PROJECTION_REQUIRED_REVIEW_CHECKS,
      rationale: 'Caller-supplied targetId drift must fail DEC-0023 revalidation.',
      audit: audit(
        'evt-gold-dec0027-target-id-drift',
        normalizationReviewer.principalId
      )
    }),
...
for (const [label, value] of [
  ['raw-serf', 'SERF'],
  ['display-name', 'Southeast Research and Demonstration Farm'],
  ['arbitrary-farm-id', driftTargetId]
]) {
  expectTargetRefProjectionError(
    () =>
      publishAgronomicContextTargetRefFarmInstanceProjectionReviewDecision({
        ...
        version: '1',
        projection: buildTargetRefFarmProjection({
          targetRefProjection: { field: 'farmId', value }
        }),
        disposition: 'ACCEPT_CONTEXT_TARGET_REF_FARM_INSTANCE_PROJECTION',
        reviewerPrincipal: normalizationReviewer,
        authorizationDecisionAuditRefs: [
  ...
  expectTargetRefProjectionError(
    () =>
      publishAgronomicContextTargetRefFarmInstanceProjectionReviewDecision({
        ledger: env.ledger,
        sourceRegistry: env.sourceRegistry,
        logicalId: `review.gold.dec0027.wrong-field-${field}`,
        version: '1',
        projection: buildTargetRefFarmProjection({
          targetRefProjection: { field, value: exactTargetId }
        }),
        disposition: 'ACCEPT_CONTEXT_TARGET_REF_FARM_INSTANCE_PROJECTION',
        reviewerPrincipal: normalizationReviewer,
        authorizationDecisionAuditRefs: [
        ...
        confirmedChecks:
          AGRONOMIC_CONTEXT_TARGET_REF_FARM_INSTANCE_PROJECTION_REQUIRED_REVIEW_CHECKS,
        rationale: 'Only targetRef.farmId is authorized.',
        audit: audit(
          `evt-gold-dec0027-wrong-field-${field}`,
          normalizationReviewer.principalId
        )
      }),
...
for (const [key, value] of [
  ...
  ['seasonId', '2011'],
  ['zoneId', 'zone-a'],
  ['geometryRef', exactTargetId],
  ['geometry', { type: 'Polygon', coordinates: [] }],
  ['crossProviderEquivalentTargetId', 'deere-farm-1'],
  ...
  expectTargetRefProjectionError(
    () =>
      publishAgronomicContextTargetRefFarmInstanceProjectionReviewDecision({
        ledger: env.ledger,
        sourceRegistry: env.sourceRegistry,
        logicalId: `review.gold.dec0027.forbidden-${key}`,
        version: '1',
        projection: {
          ...buildTargetRefFarmProjection(),
          [key]: value
        },
        disposition: 'ACCEPT_CONTEXT_TARGET_REF_FARM_INSTANCE_PROJECTION',
        reviewerPrincipal: normalizationReviewer,
        ...
        confirmedChecks:
          AGRONOMIC_CONTEXT_TARGET_REF_FARM_INSTANCE_PROJECTION_REQUIRED_REVIEW_CHECKS,
        rationale: 'Forbidden target/world widening must fail closed.',
        audit: audit(
          `evt-gold-dec0027-forbidden-${key}`,
          normalizationReviewer.principalId
        )
      }),
...
const incompleteChecks =
  AGRONOMIC_CONTEXT_TARGET_REF_FARM_INSTANCE_PROJECTION_REQUIRED_REVIEW_CHECKS
    .filter((check) => check !== 'NO_GEOMETRY_OR_GEOMETRY_REF_INFERENCE');
expectTargetRefProjectionError(
  () =>
    publishAgronomicContextTargetRefFarmInstanceProjectionReviewDecision({
      ledger: env.ledger,
      sourceRegistry: env.sourceRegistry,
      logicalId: 'review.gold.dec0027.incomplete-review',
      version: '1',
      projection: buildTargetRefFarmProjection(),
      disposition: 'ACCEPT_CONTEXT_TARGET_REF_FARM_INSTANCE_PROJECTION',
      reviewerPrincipal: normalizationReviewer,
      ...
      rationale: 'Incomplete review cannot authorize projection.',
      audit: audit(
        'evt-gold-dec0027-incomplete-review',
        normalizationReviewer.principalId
      )
    }),
...
const unauthorizedTargetRefReviewer = createPrincipal({
  principalId: 'gold-dec0027-unauthorized-reviewer',
  type: 'USER',
  organizationId: 'org-a',
  tenantId: 'tenant-a'
});
expectTargetRefProjectionError(
  () =>
    publishAgronomicContextTargetRefFarmInstanceProjectionReviewDecision({
      ledger: env.ledger,
      sourceRegistry: env.sourceRegistry,
      logicalId: 'review.gold.dec0027.unauthorized-reviewer',
      version: '1',
      projection: buildTargetRefFarmProjection(),
      disposition: 'ACCEPT_CONTEXT_TARGET_REF_FARM_INSTANCE_PROJECTION',
      reviewerPrincipal: unauthorizedTargetRefReviewer,
      authorizationDecisionAuditRefs: [
        parentSourceAuthorization.auth.ref,
        semanticSourceAuthorization.auth.ref,
        targetIdentityAuthorization.auth.ref
      ...
      rationale: 'Another reviewer cannot borrow predecessor-source authorizations.',
      audit: audit(
        'evt-gold-dec0027-unauthorized-reviewer',
        unauthorizedTargetRefReviewer.principalId
      )
    }),
  'AGRONOMIC_CONTEXT_TARGET_REF_FARM_INSTANCE_PROJECTION_REVIEW_AUTHORIZATION_INVALID'
);

const rejectedTargetRefReview =
  publishAgronomicContextTargetRefFarmInstanceProjectionReviewDecision({
    ledger: env.ledger,
    sourceRegistry: env.sourceRegistry,
    logicalId: 'review.gold.dec0027.rejected',
    version: '1',
    projection: buildTargetRefFarmProjection(),
    disposition: 'REJECT_CONTEXT_TARGET_REF_FARM_INSTANCE_PROJECTION',
    reviewerPrincipal: normalizationReviewer,
    authorizationDecisionAuditRefs: [
      parentSourceAuthorization.auth.ref,
      semanticSourceAuthorization.auth.ref,
    ...
    audit: audit(
      'evt-gold-dec0027-rejected-review',
      normalizationReviewer.principalId
    )
  });
expectTargetRefProjectionError(
  () =>
    publishAgronomicContextTargetRefFarmInstanceProjectionCompilation({
      ledger: env.ledger,
      sourceRegistry: env.sourceRegistry,
      logicalId: 'compilation.gold.dec0027.rejected',
      version: '1',
      compilation: buildTargetRefFarmProjectionCompilation(
        buildTargetRefFarmProjection(),
        rejectedTargetRefReview.ref
      ),
      audit: audit(
        'evt-gold-dec0027-rejected-publication',
        normalizationReviewer.principalId
      )
    }),
...
const reviewMismatchProjection = buildTargetRefFarmProjection({
...
expectTargetRefProjectionError(
  () =>
    publishAgronomicContextTargetRefFarmInstanceProjectionCompilation({
      ledger: env.ledger,
      sourceRegistry: env.sourceRegistry,
      logicalId: 'compilation.gold.dec0027.review-mismatch',
      version: '1',
      compilation: buildTargetRefFarmProjectionCompilation(
        reviewMismatchProjection,
        targetRefReview.ref
      ),
      audit: audit(
        'evt-gold-dec0027-review-mismatch-publication',
        normalizationReviewer.principalId
      )
    }),
...
function expectFarmTargetBindingError(fn, code) {
  assert.throws(fn, (error) => {
    assert.ok(
      error instanceof AgronomicDecisionProblemFarmTargetBindingCompilationError,
      `expected DEC-0032 typed error, got ${error?.constructor?.name ?? 'none'}`
    );
    assert.equal(error.code, code);
    return true;
  });
...
function buildFarmTargetBinding(overrides = {}) {
  return {
    contractVersion:
      AGRONOMIC_DECISION_PROBLEM_FARM_TARGET_BINDING_CONTRACT_VERSION,
    bindingId:
      'binding.gold.sustainable-corn.decision-problem-farm-target',
    parentTargetRefFarmInstanceProjectionCompilationRef:
      targetRefPublished.ref,
    ...
    rationale:
      'Bind only exact DEC-0027 farmId into A01; deployment scope and decision intent remain explicit creator authority.',
    ...overrides
  };
}

function buildFarmTargetBindingCompilation(
  binding,
  reviewRef,
  status = 'COMPLETE'
) {
  return {
    ...
    losslessCoverage: {
      ...
      unrepresentedElements:
        status === 'COMPLETE' ? [] : ['UNRESOLVED_DEC0032_BINDING_ELEMENT']
    },
    limitations: [
      ...
      'NO_RUNTIME_OR_DECISION_RESULT_AUTHORITY'
    ]
  };
}

const farmTargetBinding = buildFarmTargetBinding();
const farmTargetBindingReview =
  publishAgronomicDecisionProblemFarmTargetBindingReviewDecision({
    ledger: env.ledger,
    sourceRegistry: env.sourceRegistry,
    logicalId:
      'review.gold.sustainable-corn.decision-problem-farm-target-binding',
    version: '1',
    binding: farmTargetBinding,
    disposition:
    ...
    confirmedChecks:
      AGRONOMIC_DECISION_PROBLEM_FARM_TARGET_BINDING_REQUIRED_REVIEW_CHECKS,
    rationale:
      'Exact DEC-0027 FARM target authority is accepted only as farmId; all deployment and decision-intent ownership remains outside source evidence.',
    audit: audit(
      'evt-gold-dec0032-binding-review',
      normalizationReviewer.principalId
    )
  });

const farmTargetBindingPublished =
  publishAgronomicDecisionProblemFarmTargetBindingCompilation({
    ledger: env.ledger,
    sourceRegistry: env.sourceRegistry,
    logicalId:
      'compilation.gold.sustainable-corn.decision-problem-farm-target-binding',
    version: '1',
    compilation:
      buildFarmTargetBindingCompilation(
        farmTargetBinding,
        farmTargetBindingReview.ref
      ),
    audit: audit(
      'evt-gold-dec0032-binding-publication',
      normalizationReviewer.principalId
    )
  });

const farmTargetBindingValidated =
  validateAgronomicDecisionProblemFarmTargetBindingCompilationAuthority({
    ledger: env.ledger,
    sourceRegistry: env.sourceRegistry,
    compilationRef: farmTargetBindingPublished.ref
  });

assert.equal(
  farmTargetBindingValidated.semanticPayload.binding
...
const decisionProblemCreator = createPrincipal({
  principalId: 'gold-dec0032-decision-problem-creator',
  type: 'USER',
  organizationId: 'org-a',
  tenantId: 'tenant-a',
  programIds: []
...
const decisionProblemCreatorRole = publishRoleAssignment({
  ledger: env.ledger,
  logicalId: 'role.gold.dec0032.decision-problem-creator',
  version: '1',
  principal: decisionProblemCreator,
  role: 'DECISION_PROBLEM_CREATOR',
  roleDefinitionVersion: 'adr-dec0032-gold-v1',
  ...
  audit: audit(
    'evt-gold-dec0032-decision-problem-role',
    decisionProblemCreator.principalId
  )
});

function createDecisionProblemAuthorization(
  logicalId,
  actor = decisionProblemCreator,
  assignment = decisionProblemCreatorRole
) {
  ...
  return recordAuthorizationDecision({
    ...
    decision,
    audit: audit(
      'evt-gold-dec0032-decision-problem-auth-' + logicalId,
      actor.principalId
    )
  });
}

const deterministicDecisionIntentFixture = {
  ...
  constraints: [
    { code: 'MACHINE_ACCEPTANCE_FIXTURE_BOUND', value: '1' }
  ],
...
const decisionProblemLogicalId =
  'dp-gold-sustainable-corn-dec0032-farm-target-binding';
const decisionProblemAuthorization =
  createDecisionProblemAuthorization(decisionProblemLogicalId);

const sourceBackedDecisionProblem =
  publishAgronomicDecisionProblemWithFarmTargetBinding({
    ledger: env.ledger,
    sourceRegistry: env.sourceRegistry,
    farmTargetBindingCompilationRef: farmTargetBindingPublished.ref,
    logicalId: decisionProblemLogicalId,
    version: '1',
    deploymentScope: {
      organizationId: 'org-a',
      tenantId: 'tenant-a'
    },
    decisionIntent: deterministicDecisionIntentFixture,
    principal: decisionProblemCreator,
    authorizationDecisionAuditRef: decisionProblemAuthorization.ref,
    audit: audit(
      'evt-gold-dec0032-decision-problem-publication',
      decisionProblemCreator.principalId
    )
  });

const sourceBackedDecisionProblemValidated =
  validateAgronomicDecisionProblemFarmTargetPublicationAuthority({
    ledger: env.ledger,
    sourceRegistry: env.sourceRegistry,
    decisionProblemRef: sourceBackedDecisionProblem.ref
  });

assert.equal(
  sourceBackedDecisionProblemValidated.semanticPayload.contractVersion,
  DECISION_PROBLEM_CONTRACT_VERSION
);
assert.deepEqual(
  sourceBackedDecisionProblemValidated.semanticPayload.targetRef,
  {
    organizationId: 'org-a',
    tenantId: 'tenant-a',
...
assert.equal(
  sourceBackedDecisionProblemValidated.semanticPayload.decisionType,
  'MACHINE_ACCEPTANCE_FIXTURE_DECISION'
);
assert.equal(
  sourceBackedDecisionProblemValidated.semanticPayload.decisionAuthorityMode,
  'RUNTIME_ONLY'
);
assert.equal(
  Object.hasOwn(
...
const alternateTargetRefPublished =
  publishAgronomicContextTargetRefFarmInstanceProjectionCompilation({
    ledger: env.ledger,
    sourceRegistry: env.sourceRegistry,
    logicalId:
      'compilation.gold.dec0032.semantic-equal-alternate-target-ref-projection',
    version: '1',
    compilation:
      buildTargetRefFarmProjectionCompilation(
        targetRefProjection,
        targetRefReview.ref
      ),
    audit: audit(
      'evt-gold-dec0032-semantic-equal-alternate-target-ref-projection',
      normalizationReviewer.principalId
    )
  });

const exactRefDriftBinding = buildFarmTargetBinding({
  parentTargetRefFarmInstanceProjectionCompilationRef:
    alternateTargetRefPublished.ref
});
expectFarmTargetBindingError(
  () =>
    publishAgronomicDecisionProblemFarmTargetBindingCompilation({
      ledger: env.ledger,
      sourceRegistry: env.sourceRegistry,
      logicalId: 'compilation.gold.dec0032.exact-ref-drift',
      version: '1',
      compilation:
        buildFarmTargetBindingCompilation(
          exactRefDriftBinding,
          farmTargetBindingReview.ref
        ),
      audit: audit(
        'evt-gold-dec0032-exact-ref-drift',
        normalizationReviewer.principalId
      )
    }),
...
for (const [label, override] of [
  ['raw-serf', { farmId: 'SERF' }],
  ['display-name', {
    farmId: 'Southeast Research and Demonstration Farm'
  }],
  ['arbitrary-farm', { farmId: 'farm-1' }],
  ['field-id', { fieldId: 'field-1' }],
  ['season-id', { seasonId: 'season-2011' }],
  ['zone-id', { zoneId: 'zone-1' }],
  ['geometry', { geometry: { type: 'Point', coordinates: [0, 0] } }],
  ['geometry-ref', { geometryRef: 'geometry-1' }],
  ['target-ref', {
    targetRef: {
      organizationId: 'org-a',
      tenantId: 'tenant-a',
  ...
  expectFarmTargetBindingError(
    () =>
      publishAgronomicDecisionProblemWithFarmTargetBinding({
        ...
        sourceRegistry: env.sourceRegistry,
        farmTargetBindingCompilationRef: farmTargetBindingPublished.ref,
        logicalId: 'dp-gold-dec0032-forbidden-' + label,
        version: '1',
        deploymentScope: {
          organizationId: 'org-a',
          tenantId: 'tenant-a'
        ...
        authorizationDecisionAuditRef: decisionProblemAuthorization.ref,
        audit: audit(
          'evt-gold-dec0032-forbidden-' + label,
          decisionProblemCreator.principalId
        ),
        ...override
      }),
...
expectFarmTargetBindingError(
  () =>
    publishAgronomicDecisionProblemWithFarmTargetBinding({
      ...
      sourceRegistry: env.sourceRegistry,
      farmTargetBindingCompilationRef: farmTargetBindingPublished.ref,
      logicalId: 'dp-gold-dec0032-target-laundering',
      version: '1',
      deploymentScope: {
        organizationId: 'org-a',
        tenantId: 'tenant-a'
      ...
      decisionIntent: {
        ...deterministicDecisionIntentFixture,
        constraints: [
          { code: 'FORBIDDEN_TARGET', fieldId: 'field-1' }
        ]
      },
      principal: decisionProblemCreator,
      authorizationDecisionAuditRef: decisionProblemAuthorization.ref,
      audit: audit(
        'evt-gold-dec0032-target-laundering',
        decisionProblemCreator.principalId
      )
    }),
...
for (const omitted of [
  ...
  const incompleteIntent = structuredClone(deterministicDecisionIntentFixture);
  delete incompleteIntent[omitted];
  expectFarmTargetBindingError(
    () =>
      publishAgronomicDecisionProblemWithFarmTargetBinding({
        ledger: env.ledger,
        sourceRegistry: env.sourceRegistry,
        farmTargetBindingCompilationRef: farmTargetBindingPublished.ref,
        logicalId: 'dp-gold-dec0032-missing-intent-' + omitted,
        version: '1',
        deploymentScope: {
          organizationId: 'org-a',
          tenantId: 'tenant-a'
        },
        decisionIntent: incompleteIntent,
        principal: decisionProblemCreator,
        authorizationDecisionAuditRef: decisionProblemAuthorization.ref,
        audit: audit(
          'evt-gold-dec0032-missing-intent-' + omitted,
          decisionProblemCreator.principalId
        )
      }),
...
expectFarmTargetBindingError(
  () =>
    publishAgronomicDecisionProblemWithFarmTargetBinding({
      ...
      sourceRegistry: env.sourceRegistry,
      farmTargetBindingCompilationRef: farmTargetBindingPublished.ref,
      logicalId: 'dp-gold-dec0032-missing-deployment-scope',
      version: '1',
      deploymentScope: {},
      decisionIntent: deterministicDecisionIntentFixture,
      principal: decisionProblemCreator,
      authorizationDecisionAuditRef: decisionProblemAuthorization.ref,
      audit: audit(
        'evt-gold-dec0032-missing-deployment-scope',
        decisionProblemCreator.principalId
      )
    }),
...
const unauthorizedBindingReviewer = createPrincipal({
  principalId: 'gold-dec0032-unauthorized-binding-reviewer',
  type: 'USER',
  organizationId: 'org-a',
  tenantId: 'tenant-a'
});
expectFarmTargetBindingError(
  () =>
    publishAgronomicDecisionProblemFarmTargetBindingReviewDecision({
      ledger: env.ledger,
      sourceRegistry: env.sourceRegistry,
      logicalId: 'review.gold.dec0032.unauthorized',
      version: '1',
      binding: buildFarmTargetBinding(),
      disposition:
        'ACCEPT_AGRONOMIC_DECISION_PROBLEM_FARM_TARGET_BINDING',
      reviewerPrincipal: unauthorizedBindingReviewer,
      authorizationDecisionAuditRefs: [
        parentSourceAuthorization.auth.ref,
        semanticSourceAuthorization.auth.ref,
        targetIdentityAuthorization.auth.ref
      ...
      rationale: 'Another reviewer cannot borrow source inspection authority.',
      audit: audit(
        'evt-gold-dec0032-unauthorized-binding-reviewer',
        unauthorizedBindingReviewer.principalId
      )
    }),
  'AGRONOMIC_DECISION_PROBLEM_FARM_TARGET_BINDING_REVIEW_AUTHORIZATION_INVALID'
);

const incompleteBindingChecks =
  AGRONOMIC_DECISION_PROBLEM_FARM_TARGET_BINDING_REQUIRED_REVIEW_CHECKS
    .filter((check) => check !== 'NO_CALLER_FARM_ID_AUTHORITY');
expectFarmTargetBindingError(
  () =>
    publishAgronomicDecisionProblemFarmTargetBindingReviewDecision({
      ledger: env.ledger,
      sourceRegistry: env.sourceRegistry,
      logicalId: 'review.gold.dec0032.incomplete',
      version: '1',
      binding: buildFarmTargetBinding(),
      disposition:
        'ACCEPT_AGRONOMIC_DECISION_PROBLEM_FARM_TARGET_BINDING',
      ...
      confirmedChecks: incompleteBindingChecks,
      rationale: 'Incomplete binding review must fail closed.',
      audit: audit(
        'evt-gold-dec0032-incomplete-binding-review',
        normalizationReviewer.principalId
      )
    }),
...
const rejectedBindingReview =
  publishAgronomicDecisionProblemFarmTargetBindingReviewDecision({
    ledger: env.ledger,
    sourceRegistry: env.sourceRegistry,
    logicalId: 'review.gold.dec0032.rejected',
    version: '1',
    binding: buildFarmTargetBinding(),
    disposition:
    ...
    confirmedChecks: [],
    rationale: 'Rejected binding cannot authorize publication.',
    audit: audit(
      'evt-gold-dec0032-rejected-binding-review',
      normalizationReviewer.principalId
    )
  });
expectFarmTargetBindingError(
  () =>
    publishAgronomicDecisionProblemFarmTargetBindingCompilation({
      ledger: env.ledger,
      sourceRegistry: env.sourceRegistry,
      logicalId: 'compilation.gold.dec0032.rejected',
      version: '1',
      compilation:
        buildFarmTargetBindingCompilation(
          buildFarmTargetBinding(),
          rejectedBindingReview.ref
        ),
      audit: audit(
        'evt-gold-dec0032-rejected-binding-publication',
        normalizationReviewer.principalId
      )
    }),
...
const wrongLogicalIdAuthorization =
  createDecisionProblemAuthorization('dp-gold-dec0032-other-id');
assert.throws(() =>
  publishAgronomicDecisionProblemWithFarmTargetBinding({
    ledger: env.ledger,
    sourceRegistry: env.sourceRegistry,
    farmTargetBindingCompilationRef: farmTargetBindingPublished.ref,
    logicalId: 'dp-gold-dec0032-wrong-auth-id',
    version: '1',
    deploymentScope: {
      organizationId: 'org-a',
      tenantId: 'tenant-a'
    ...
    decisionIntent: deterministicDecisionIntentFixture,
    principal: decisionProblemCreator,
    authorizationDecisionAuditRef: wrongLogicalIdAuthorization.ref,
    audit: audit(
      'evt-gold-dec0032-wrong-auth-id',
      decisionProblemCreator.principalId
    )
  })
);

const genericDecisionProblemLogicalId =
  'dp-gold-dec0032-generic-matching';
const genericDecisionProblemAuthorization =
  createDecisionProblemAuthorization(genericDecisionProblemLogicalId);
const genericMatchingDecisionProblem = publishDecisionProblem({
  ledger: env.ledger,
  logicalId: genericDecisionProblemLogicalId,
  version: '1',
  problem: {
    ...
    ...structuredClone(deterministicDecisionIntentFixture)
  },
  principal: decisionProblemCreator,
  authorizationDecisionAuditRef: genericDecisionProblemAuthorization.ref,
  audit: audit(
    'evt-gold-dec0032-generic-matching',
    decisionProblemCreator.principalId
  )
});
expectFarmTargetBindingError(
  () =>
    validateAgronomicDecisionProblemFarmTargetPublicationAuthority({
      ledger: env.ledger,
      sourceRegistry: env.sourceRegistry,
      decisionProblemRef: genericMatchingDecisionProblem.ref
    }),
...
const decisionProblemRecords = env.ledger.exportSnapshot().records
  .filter((record) => record.ref.kind === 'DecisionProblem');
assert.equal(decisionProblemRecords.length, 2);
assert.ok(
  decisionProblemRecords.some((record) =>
    record.ref.semanticHash === sourceBackedDecisionProblem.ref.semanticHash
  )
);
assert.ok(
  decisionProblemRecords.some((record) =>
    record.ref.semanticHash === genericMatchingDecisionProblem.ref.semanticHash
  )
);
...
console.log(JSON.stringify({
  ok: true,
  authority: 'AgronomicDecisionProblemFarmTargetBindingPublication',
  goldKind: 'PUBLIC_REAL_SOURCE_TARGET_PLUS_DETERMINISTIC_A01_FIXTURE',
  parentTargetRefFarmInstanceProjection: targetRefPublished.ref,
  bindingCompilationRef: farmTargetBindingPublished.ref,
  decisionProblemRef: sourceBackedDecisionProblem.ref,
  sourceBackedFarmId: exactTargetId,
  targetRef: sourceBackedDecisionProblemValidated.semanticPayload.targetRef,
  decisionIntentClassification: 'DETERMINISTIC_MACHINE_ACCEPTANCE_FIXTURE_NOT_SOURCE_DERIVED',
  deploymentScopeClassification: 'EXPLICIT_A01_CREATOR_OWNED',
  farmIdAuthorityClassification: 'EXACT_DEC_0027_SOURCE_BACKED',
  genericA01ContractModified: false,
  callerFarmIdAuthorityUsed: false,
  sourceDerivedDecisionIntentUsed: false,
  fieldSeasonZoneAuthorityCreated: false,
  geometryAuthorityCreated: false,
  contextManifestCreated: false,
  evidenceCutoffAuthorityCreated: false,
  negativeCases: [
    'SEMANTICALLY_EQUAL_DIFFERENT_DEC_0027_REF_SUBSTITUTION_DENIED',
    'RAW_SERF_FARM_ID_OVERRIDE_DENIED',
    'DISPLAY_NAME_FARM_ID_OVERRIDE_DENIED',
    'ARBITRARY_FARM_ID_OVERRIDE_DENIED',
    'FIELD_SEASON_ZONE_INJECTION_DENIED',
    'GEOMETRY_INJECTION_DENIED',
    'CALLER_TARGET_REF_DENIED',
    'TARGET_IDENTITY_LAUNDERING_IN_DECISION_INTENT_DENIED',
    'MISSING_DECISION_INTENT_NOT_DERIVED_FROM_SOURCE',
    'MISSING_DEPLOYMENT_SCOPE_NOT_INFERRED_FROM_SOURCE',
    'UNAUTHORIZED_BINDING_REVIEWER_DENIED',
    'INCOMPLETE_BINDING_REVIEW_DENIED',
    'REJECTED_BINDING_REVIEW_DENIED',
    'WRONG_DECISION_PROBLEM_LOGICAL_ID_AUTHORIZATION_DENIED',
    'GENERIC_MATCHING_A01_DECISION_PROBLEM_WITHOUT_DEC0032_PROOF_DENIED'
  ],
