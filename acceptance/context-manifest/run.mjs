...
test('material DecisionProblem change creates a new manifest semantic identity even for same context members', () => {
  const ledger = freshLedger();
  const datum = publishDatum(ledger);
  const aProblem = publishProblem(ledger, 'dp-manifest-a');
  const bProblem = publishProblem(ledger, 'dp-manifest-b', problemInput({ objective: { code: 'MINIMIZE_IRRIGATION_WATER' } }));
...
test('later ContextDatum version does not rewrite historical manifest membership', () => {
  const ledger = freshLedger();
  const problem = publishProblem(ledger);
  const oldDatum = publishDatum(ledger, 'cd-history', undefined, undefined, '1');
  const oldManifest = publishManifest(ledger, { logicalId: 'cm-history', version: '1', decisionProblem: problem, datumRefs: [oldDatum.ref] });
  publishDatum(ledger, 'cd-history', undefined, undefined, '2');
