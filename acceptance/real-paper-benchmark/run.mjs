...
const falseAccept = summarizeRealPaperBenchmark({
  ...
  papers: [{ paperId: 'paper-a', candidates: [{ candidateKey: 'a1', claimType: 'PARAMETER', compilerStatus: 'REVIEWABLE', automatedStatus: 'AUTO_ACCEPTED', referenceDisposition: 'REJECT_SOURCE_FAITHFUL', defectCodes: ['TEMPORAL_WINDOW_OMITTED'] }] }]
});
...
const incomplete = summarizeRealPaperBenchmark({
  ...
  papers: [{ paperId: 'paper-a', candidates: [{ candidateKey: 'a1', claimType: 'PARAMETER', compilerStatus: 'REVIEWABLE', automatedStatus: 'AUTO_ACCEPTED', referenceDisposition: null, defectCodes: [] }] }]
});
...
assert.throws(() => summarizeRealPaperBenchmark({
  ...
  papers: [{ paperId: 'paper-a', candidates: [{ candidateKey: 'x', claimType: null, compilerStatus: 'INVALID', automatedStatus: 'AUTO_REJECTED', referenceDisposition: null, defectCodes: [] }] }]
}), (error) => error instanceof RealPaperBenchmarkError && error.code === 'INVALID_BENCHMARK_RUN');

console.log(JSON.stringify({ total: 5, passed: 5, failed: 0 }, null, 2));
