...
assert.equal(world.providerHash, 'sha256:885d882859f464287c606f351bcff06c5fa56b8a509a5372d74124e159286da7');
...
const geoxTargetAuthority = JSON.parse(readFileSync(
  new URL('./geox-kbs-t4r1-authority-export.json', import.meta.url),
  'utf8'
));
assert.equal(geoxTargetAuthority.source_main_sha, 'd67a2b3cce037c1eaad4d7d051d1f6a11eb09fc3');
assert.equal(geoxTargetAuthority.geox_target.field_id, 'field_kbs_mcse_t4r1');
...
const message = createIntegrationMessage({
  ...
  messageId: 'geox-kbs-t4r1-target-correspondence-1',
  authorityRefs,
  payload: {
    provider_target: {
...
assert.equal(projection.provider_target.experiment_locator, 'https://lter.kbs.msu.edu/research/long-term-experiments/main-cropping-system-experiment/');
assert.equal(projection.provider_target.treatment_code, 'T4');
assert.equal(projection.provider_target.replicate_code, 'R1');
assert.equal(projection.provider_target.crop_code, 'corn');
assert.equal(projection.provider_target.hybrid_code, '43-96P');
assert.equal(projection.provider_target.planting_observation_id, '6974');
assert.equal(projection.geox_target.field_id, 'field_kbs_mcse_t4r1');
assert.equal(projection.geox_target.zone_id, 'zone_kbs_mcse_t4r1_crop_formal_v1');
...
assert.equal(projection.consumer_authority_pin.classification, 'PINNED_CONSUMER_AUTHORITY_EXPORT_QUALIFICATION_ONLY');
...
console.log(JSON.stringify({
  ok: true,
  milestone: 'PRODUCTIZATION_KBS_T4R1_CROSS_NAMESPACE_TARGET_CORRESPONDENCE_V1',
  adrTarget: {
