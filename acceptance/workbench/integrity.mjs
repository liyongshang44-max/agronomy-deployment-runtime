...
test('knowledge.inspect without source.read cannot expose Source/Claim evidence in Agronomist Workbench', () => {
  ...
  assert.throws(() => createInspectionAuthorization(base, {
    principal,
...
test('cross-tenant workbench principal can never read target Context merely because knowledge visibility is explicitly granted', () => {
  ...
  const foreign = createPrincipal({
    principalId: 'foreign-agronomist-a11',
    type: 'USER',
  ...
  const role = publishRoleAssignment({
    ...
    logicalId: 'role.a11.foreign-explicit-target-scope',
    version: '1',
    principal: foreign,
    role: 'A11_FOREIGN_EXPLICIT_READER',
  ...
  const inspection = createInspectionAuthorization(world, {
    principal: foreign,
    roleAssignments: [role],
    visibilityPrincipalId: foreign.principalId
  });
...
test('A11 case and queue projection are pure reads and do not mutate authority records/audit/lineage', () => {
  const world = createWorkbenchWorld('pure');
  const before = world.env.ledger.exportSnapshot();
  const c = projectCase(world);
  buildAgronomistEscalationQueue({ cases: [c], includeNoReviewCandidates: true });
...
test('queue rejects tampered workbench case projection instead of trusting UI fields', () => {
  const world = createWorkbenchWorld('tamper-case');
  const tampered = {
    ...world.workbenchCase,
    classification: 'AGRONOMIST_REVIEW_REQUIRED',
    reviewRequired: true
  };
  assert.throws(() => buildAgronomistEscalationQueue({ cases: [tampered] }), (error) => error?.code === 'WORKBENCH_CASE_HASH_MISMATCH');
});
...
test('later QualifiedKnowledge revocation blocks current A11 case but exact historical case remains replayable', () => {
  ...
  q.qualification.revokeQualifiedKnowledgeUse({
    revocationLogicalId: 'revocation.a11.history',
    revocationVersion: '1',
    ...
    reasonCodes: ['A11_HISTORICAL_CASE_FIXTURE'],
