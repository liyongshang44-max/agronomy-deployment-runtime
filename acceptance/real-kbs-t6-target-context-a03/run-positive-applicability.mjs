...
function audit(principal, suffix, occurredAt = '2026-09-04T07:16:00.000Z') {
  seq += 1;
  return {
    eventId: `evt-kbs-t6-positive-${seq}-${suffix}`,
    occurredAt,
    actor: { type: principal.type ?? 'USER', id: principal.principalId ?? principal.id },
    details: {
      suite: 'real-kbs-t6-positive-applicability',
...
const source = sourceRegistry.registerSource({
  logicalId: 'source.real-kbs-t6-positive.protocol',
  version: '1',
  sourceType: 'PROTOCOL',
...
const artifact = sourceRegistry.materializeArtifact({
  logicalId: 'artifact.real-kbs-t6-positive.prohibition-excerpts',
  version: '1',
...
const compilerDefinition = createDeterministicCompilerDefinition({
  ledger,
  logicalId: 'compiler.real-kbs-t6-positive',
  version: '1',
  compilerId: 'adr.real-kbs-t6-positive.curated',
...
const compilation = new ScientificCompiler({ ledger, sourceRegistry }).materializeCompilationProposal({
  compilationLogicalId: 'compilation.real-kbs-t6-positive.no-nitrogen',
  version: '1',
...
const reviewPolicy = publishKnowledgeGovernancePolicy({
  ledger,
  logicalId: 'policy.kbs-t6-positive.source-review',
  version: '1',
...
const reviewed = new SourceFaithfulReviewService({ ledger }).reviewCandidate({
  reviewLogicalId: 'review.real-kbs-t6-positive.no-nitrogen',
  reviewVersion: '1',
  ...
  claimLogicalId: 'claim.real-kbs-t6-positive.no-nitrogen',
  claimVersion: '1',
  sourceContextLogicalId: 'source-context.real-kbs-t6-positive.no-nitrogen',
  sourceContextVersion: '1',
  audit: audit(reviewer, 'source-faithful-review', '2026-09-04T07:08:00.000Z')
});

const approver = createPrincipal({
  principalId: 'kbs-t6-positive-scientific-approver', type: 'USER', ...OWNERSHIP
});
const approverRole = publishBuiltinRoleAssignment({
  ledger,
  logicalId: 'role.kbs-t6-positive.scientific-approver',
  version: '1',
  principal: approver,
...
const qualificationPolicy = publishKnowledgeGovernancePolicy({
  ledger,
  logicalId: 'policy.kbs-t6-positive.qualification',
  version: '1',
...
const qualificationDecision = qualificationService.recordQualificationDecision({
  decisionLogicalId: 'qualification.real-kbs-t6-positive.no-nitrogen',
  decisionVersion: '1',
...
const knowledge = qualificationService.publishQualifiedKnowledge({
  qualifiedKnowledgeLogicalId: 'knowledge.real-kbs-t6-positive.no-nitrogen',
...
const releaseManager = createPrincipal({
  principalId: 'kbs-t6-positive-release-manager', type: 'USER', ...OWNERSHIP, programIds: ['pilot-a']
});
const releaseRole = publishRoleAssignment({
  ledger,
  logicalId: 'role.kbs-t6-positive.release-manager',
  version: '1',
  principal: releaseManager,
  role: 'KNOWLEDGE_RELEASE_MANAGER',
  roleDefinitionVersion: 'kbs-t6-positive-v1',
...
const releasePolicy = publishKnowledgeGovernancePolicy({
  ledger,
  logicalId: 'policy.kbs-t6-positive.release-member',
  version: '1',
...
const release = new KnowledgeReleaseService({ ledger }).publishRelease({
  logicalId: 'release.real-kbs-t6-positive',
  version: '1',
  memberEntitlements: [{
...
const profileManager = createPrincipal({
  principalId: 'kbs-t6-positive-profile-manager', type: 'USER', ...OWNERSHIP, programIds: ['pilot-a']
});
const profileRole = publishRoleAssignment({
  ledger,
  logicalId: 'role.kbs-t6-positive.profile-manager',
  version: '1',
...
const profileLogicalId = 'runtime-profile.real-kbs-t6-positive';
...
const deploymentManager = createPrincipal({
  principalId: 'kbs-t6-positive-deployment-manager', type: 'USER', ...OWNERSHIP, programIds: ['pilot-a']
});
const deploymentRole = publishBuiltinRoleAssignment({
  ledger,
  logicalId: 'role.kbs-t6-positive.deployment-manager',
  version: '1',
...
const deploymentLogicalId = 'deployment.real-kbs-t6-positive';
...
const runtimePrincipal = createPrincipal({
  principalId: 'kbs-t6-positive-runtime-service', type: 'SERVICE_ACCOUNT', ...OWNERSHIP, programIds: ['pilot-a']
});
const runtimeRole = publishBuiltinRoleAssignment({
  ledger,
  logicalId: 'role.kbs-t6-positive.runtime-service',
  version: '1',
...
const retrieval = executeKnowledgeRetrieval({
  ledger,
  logicalId: 'retrieval.real-kbs-t6-positive',
  version: '1',
...
const applicability = assessKnowledgeApplicability({
  ledger,
  logicalId: 'applicability.real-kbs-t6-positive',
  version: '1',
