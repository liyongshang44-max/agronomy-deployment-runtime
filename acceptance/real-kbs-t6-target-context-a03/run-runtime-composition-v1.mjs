...
function audit(principal, suffix, occurredAt = '2026-09-04T07:16:00.000Z') {
  seq += 1;
  return {
    eventId: `evt-kbs-t6-runtime-v1-${seq}-${suffix}`,
    occurredAt,
    actor: { type: principal.type ?? 'USER', id: principal.principalId ?? principal.id },
    details: {
      suite: 'real-kbs-t6-runtime-composition-v1',
...
const source = sourceRegistry.registerSource({
  logicalId: 'source.real-kbs-t6-runtime-v1.protocol',
  version: '1',
  sourceType: 'PROTOCOL',
  ...
  metadata: { originalPdfPage: 10, benchmarkClass: 'REAL_SOURCE_RUNTIME_T6' },
  audit: audit({ type: 'USER', id: 'kbs-t6-runtime-source-curator' }, 'source', '2026-09-04T07:01:00.000Z')
});
const artifact = sourceRegistry.materializeArtifact({
  logicalId: 'artifact.real-kbs-t6-runtime-v1.prohibition-excerpts',
  version: '1',
  ...
  audit: audit({ type: 'USER', id: 'kbs-t6-runtime-source-curator' }, 'artifact', '2026-09-04T07:02:00.000Z')
});
const compilerDefinition = createDeterministicCompilerDefinition({
  ledger,
  logicalId: 'compiler.real-kbs-t6-runtime-v1',
  version: '1',
  compilerId: 'adr.real-kbs-t6-runtime-v1.curated',
  implementationVersion: '1',
  configuration: { sourcePolicy: 'CURATED_SOURCE_FAITHFUL_GOLD_FIXTURE', locatorScheme: 'PDF_PAGE_TEXT_V1' },
  audit: audit({ type: 'SERVICE_ACCOUNT', id: 'kbs-t6-runtime-compiler' }, 'compiler', '2026-09-04T07:03:00.000Z')
});
const compilation = new ScientificCompiler({ ledger, sourceRegistry }).materializeCompilationProposal({
  compilationLogicalId: 'compilation.real-kbs-t6-runtime-v1.no-nitrogen',
  version: '1',
  ...
  proposal: {
    ...
    runMetadata: { benchmark: 'REAL_KBS_T6_RUNTIME_COMPOSITION_V1' }
  },
  audit: audit({ type: 'SERVICE_ACCOUNT', id: 'kbs-t6-runtime-compiler' }, 'compilation', '2026-09-04T07:04:00.000Z')
});

const reviewer = createPrincipal({ principalId: 'kbs-t6-runtime-v1-reviewer', type: 'USER', ...OWNERSHIP });
const reviewerRole = publishBuiltinRoleAssignment({
  ledger,
  logicalId: 'role.kbs-t6-runtime-v1.reviewer',
  version: '1',
  principal: reviewer,
...
const reviewPolicy = publishKnowledgeGovernancePolicy({
  ledger,
  logicalId: 'policy.kbs-t6-runtime-v1.source-review',
  version: '1',
...
const reviewed = new SourceFaithfulReviewService({ ledger }).reviewCandidate({
  reviewLogicalId: 'review.real-kbs-t6-runtime-v1.no-nitrogen',
  reviewVersion: '1',
  ...
  claimLogicalId: 'claim.real-kbs-t6-runtime-v1.no-nitrogen',
  claimVersion: '1',
  sourceContextLogicalId: 'source-context.real-kbs-t6-runtime-v1.no-nitrogen',
  sourceContextVersion: '1',
  audit: audit(reviewer, 'source-faithful-review', '2026-09-04T07:08:00.000Z')
});

const approver = createPrincipal({ principalId: 'kbs-t6-runtime-v1-scientific-approver', type: 'USER', ...OWNERSHIP });
const approverRole = publishBuiltinRoleAssignment({
  ledger,
  logicalId: 'role.kbs-t6-runtime-v1.scientific-approver',
  version: '1',
  principal: approver,
...
const qualificationPolicy = publishKnowledgeGovernancePolicy({
  ledger,
  logicalId: 'policy.kbs-t6-runtime-v1.qualification',
  version: '1',
...
const qualificationDecision = qualificationService.recordQualificationDecision({
  decisionLogicalId: 'qualification.real-kbs-t6-runtime-v1.no-nitrogen',
  decisionVersion: '1',
...
const knowledge = qualificationService.publishQualifiedKnowledge({
  qualifiedKnowledgeLogicalId: 'knowledge.real-kbs-t6-runtime-v1.no-nitrogen',
...
const releaseManager = createPrincipal({
  principalId: 'kbs-t6-runtime-v1-release-manager', type: 'USER', ...OWNERSHIP, programIds: ['pilot-a']
});
const releaseRole = publishRoleAssignment({
  ledger,
  logicalId: 'role.kbs-t6-runtime-v1.release-manager',
  version: '1',
  principal: releaseManager,
  role: 'KNOWLEDGE_RELEASE_MANAGER',
  roleDefinitionVersion: 'kbs-t6-runtime-v1-v1',
...
const releasePolicy = publishKnowledgeGovernancePolicy({
  ledger,
  logicalId: 'policy.kbs-t6-runtime-v1.release-member',
  version: '1',
...
const release = new KnowledgeReleaseService({ ledger }).publishRelease({
  logicalId: 'release.real-kbs-t6-runtime-v1',
  version: '1',
  memberEntitlements: [{
...
const profileManager = createPrincipal({
  principalId: 'kbs-t6-runtime-v1-profile-manager', type: 'USER', ...OWNERSHIP, programIds: ['pilot-a']
});
const profileRole = publishRoleAssignment({
  ledger,
  logicalId: 'role.kbs-t6-runtime-v1.profile-manager',
  version: '1',
...
const profileLogicalId = 'runtime-profile.real-kbs-t6-runtime-v1';
...
const deploymentManager = createPrincipal({
  principalId: 'kbs-t6-runtime-v1-deployment-manager', type: 'USER', ...OWNERSHIP, programIds: ['pilot-a']
});
const deploymentRole = publishBuiltinRoleAssignment({
  ledger,
  logicalId: 'role.kbs-t6-runtime-v1.deployment-manager',
  version: '1',
...
const deploymentLogicalId = 'deployment.real-kbs-t6-runtime-v1';
...
const runtimePrincipal = createPrincipal({
  principalId: 'kbs-t6-runtime-v1-runtime-service', type: 'SERVICE_ACCOUNT', ...OWNERSHIP, programIds: ['pilot-a']
});
const runtimeRole = publishBuiltinRoleAssignment({
  ledger,
  logicalId: 'role.kbs-t6-runtime-v1.runtime-service',
  version: '1',
...
const retrieval = executeKnowledgeRetrieval({
  ledger,
  logicalId: 'retrieval.real-kbs-t6-runtime-v1',
  version: '1',
...
const applicability = assessKnowledgeApplicability({
  ledger,
  logicalId: 'applicability.real-kbs-t6-runtime-v1',
  version: '1',
...
const binding = publishRuntimeBinding({
  ledger,
  logicalId: 'runtime-binding.real-kbs-t6-runtime-v1',
  version: '1',
  runtimeEligibilityRef: eligibility.ref,
...
assert.equal(validatedBinding.semanticPayload.logicalTime, new Date(KBS_T6_LOGICAL_TIME).toISOString());
assert.equal(validatedBinding.semanticPayload.evidenceCutoff, world.validatedManifest.semanticPayload.evidenceCutoff);
assert.deepEqual(validatedBinding.semanticPayload.knowledgeBindings, [{
  knowledgeRef: knowledge.ref,
  applicabilityAssessmentRef: applicability.ref
}]);
assert.equal(
  validatedBinding.semanticPayload.correctnessClaim,
...
assert.equal(records.filter((record) => record.ref.kind === 'RuntimeBinding').length, 1);

console.log(JSON.stringify({
  ok: true,
  milestone: 'REAL_WORLD_HETEROGENEITY_NITROGEN_A08_R01_R03_D01',
  ...
  targetWorld: {
    ...
    logicalTime: new Date(KBS_T6_LOGICAL_TIME).toISOString(),
  ...
  runtimePlan: {
    ...
    runtimePlanHash: plan.runtimePlanHash,
  ...
  dec0034RequiredForRuntimeComposition: false,
