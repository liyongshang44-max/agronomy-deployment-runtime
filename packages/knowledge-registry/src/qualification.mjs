...
function normalizeReasonCodes(values, { required = false } = {}) {
  ...
  const normalized = [...new Set(raw.map((value) => requiredText(value, 'reasonCode')))].sort();
  if (required && normalized.length === 0) {
    throw new ScientificQualificationError(
      'QUALIFICATION_REASON_REQUIRED',
      'prohibited/revoked scientific use requires a reason code'
    );
  }
  return deepFreeze(normalized);
}
...
function canonicalEqual(left, right) {
  return semanticHash('ADR-K04-CANONICAL-COMPARE', left)
    === semanticHash('ADR-K04-CANONICAL-COMPARE', right);
}
...
function hasExactIdentity(ledger, kind, logicalId, version) {
  const exactVersion = requiredText(version, 'version');
  return ledger.listVersions(kind, requiredText(logicalId, 'logicalId'))
    .some((ref) => ref.version === exactVersion);
}

function refSetEquals(leftRefs, rightRefs) {
  const left = [...new Set(leftRefs.map(exactRefKey))].sort();
  const right = [...new Set(rightRefs.map(exactRefKey))].sort();
  return canonicalEqual(left, right);
}

function normalizeSupersessionInput({ supersedesDecisionRef, supersedesDecisionRefs }) {
  if (supersedesDecisionRef && supersedesDecisionRefs !== undefined) {
    throw new ScientificQualificationError(
      'AMBIGUOUS_QUALIFICATION_SUPERSESSION',
      'use supersedesDecisionRefs or legacy supersedesDecisionRef, not both'
    );
  }
  const raw = supersedesDecisionRefs !== undefined
    ? supersedesDecisionRefs
    : (supersedesDecisionRef ? [supersedesDecisionRef] : []);
  if (!Array.isArray(raw)) {
    throw new ScientificQualificationError(
      'INVALID_QUALIFICATION_SUPERSESSION',
      'supersedesDecisionRefs must be an array'
    );
  }
  const normalized = raw.map((ref) => assertAuthorityRef(ref));
  const keys = normalized.map(exactRefKey);
  if (new Set(keys).size !== keys.length) {
    throw new ScientificQualificationError(
      'INVALID_QUALIFICATION_SUPERSESSION',
      'supersedesDecisionRefs cannot contain duplicate exact refs'
    );
  }
  return deepFreeze([...normalized].sort((left, right) => exactRefKey(left).localeCompare(exactRefKey(right))));
}

function decisionSupersessionRefs(decision) {
  const refs = decision.semanticPayload?.supersedesDecisionRefs ?? [];
  if (!Array.isArray(refs)) {
    throw new ScientificQualificationError(
      'INVALID_QUALIFICATION_DECISION',
      'ScientificQualificationDecision.supersedesDecisionRefs must be an array'
    );
  }
  return refs.map((ref) => assertAuthorityRef(ref));
}
...
function assertFinalContextMatchesCandidate(candidateFamilies, finalFamilies) {
  ...
  if (!canonicalEqual(candidateNames, finalNames)) {
    throw new ScientificQualificationError(
      'SOURCE_FAITHFUL_CONTEXT_INVALID',
      'final SourceContext families differ from reviewed candidate families'
    );
  }
  ...
  for (const family of candidateNames) {
    ...
    if (candidateFamily.status !== finalFamily.status) {
      throw new ScientificQualificationError(
        'SOURCE_FAITHFUL_CONTEXT_INVALID',
        `${family} reporting status differs from candidate`
      );
    }
    if (candidateFamily.status === 'NOT_REPORTED') {
      if ((candidateFamily.dimensions ?? []).length !== 0 || (finalFamily.dimensions ?? []).length !== 0) {
        throw new ScientificQualificationError(
          'SOURCE_FAITHFUL_CONTEXT_INVALID',
          `${family} NOT_REPORTED cannot contain dimensions`
        );
      }
      continue;
    }
    ...
    if (candidateDimensions.length !== finalDimensions.length) {
      throw new ScientificQualificationError(
        'SOURCE_FAITHFUL_CONTEXT_INVALID',
        `${family} final dimension count differs from candidate`
      );
    }
    ...
    for (let index = 0; index < candidateDimensions.length; index += 1) {
      ...
      try {
        ...
        throw new ScientificQualificationError(
          'SOURCE_FAITHFUL_CONTEXT_INVALID',
          `cannot reconstruct ${family}[${index}] from source-faithful candidate: ${error.message}`
        );
      }
      if (!canonicalEqual(reconstructed, finalDimension)) {
        throw new ScientificQualificationError(
          'SOURCE_FAITHFUL_CONTEXT_INVALID',
          `${family}[${index}] is not source-faithful to the reviewed candidate`
        );
      }
    }
  }
...
function assertK03ReviewAuthorization({ ledger, review, source }) {
  ...
  if (!reviewPrincipal || typeof reviewPrincipal !== 'object' || Array.isArray(reviewPrincipal)) {
    throw new ScientificQualificationError(
      'SOURCE_FAITHFUL_REVIEW_AUTHORIZATION_INVALID',
      'reviewPrincipal is missing from K03 review authority'
    );
  }
  ...
  if (decision.allowed !== true || decision.operation !== 'KNOWLEDGE_INSPECT') {
    throw new ScientificQualificationError(
      'SOURCE_FAITHFUL_REVIEW_AUTHORIZATION_INVALID',
      'K03 review requires allowed KNOWLEDGE_INSPECT authorization'
    );
  }
  if (!samePrincipalIdentity(decision.principal, reviewPrincipal)) {
    throw new ScientificQualificationError(
      'SOURCE_FAITHFUL_REVIEW_AUTHORIZATION_INVALID',
      'K03 review principal differs from authorization principal'
    );
  }

  const policy = resolveKind(
    ledger,
    decision.policyRef,
    'KnowledgeGovernancePolicy',
    'SOURCE_FAITHFUL_REVIEW_POLICY_REQUIRED'
  );
  if (policy.semanticPayload.resourceId !== sourceReviewResourceId(source.ref)) {
    throw new ScientificQualificationError(
      'SOURCE_FAITHFUL_REVIEW_AUTHORIZATION_INVALID',
      'K03 review policy is not bound to exact Source review resource'
    );
  }
  ...
  if (sourceOwnership.organizationId !== policyOwnership.organizationId
    ...
    throw new ScientificQualificationError(
      'SOURCE_FAITHFUL_REVIEW_AUTHORIZATION_INVALID',
      'K03 review policy ownership differs from Source ownership'
    );
  }
  ...
  if (recomputed.allowed !== true || recomputed.decisionHash !== decision.decisionHash) {
    throw new ScientificQualificationError(
      'SOURCE_FAITHFUL_REVIEW_AUTHORIZATION_INVALID',
      'K03 review authorization cannot be reproduced from exact F03 authority'
    );
  }
  ...
  if (!hasSourceRead) {
    throw new ScientificQualificationError(
      'SOURCE_FAITHFUL_REVIEW_AUTHORIZATION_INVALID',
      'K03 review authority lacks SOURCE_READ + KNOWLEDGE_INSPECT grant'
    );
  }
...
function assertSourceFaithfulAuthorityPair({ ledger, claimRef, sourceContextRef }) {
  ...
  const sourceContext = resolveKind(
    ledger,
    sourceContextRef,
    'SourceContext',
    'SOURCE_CONTEXT_AUTHORITY_REQUIRED'
  );

  if (claim.semanticPayload.authorityClass !== 'SOURCE_ASSERTION') {
    throw new ScientificQualificationError(
      'INVALID_SOURCE_ASSERTION_AUTHORITY',
      'Claim must be SOURCE_ASSERTION authority'
    );
  }
  if (sourceContext.semanticPayload.authorityClass !== 'SOURCE_CONTEXT') {
    throw new ScientificQualificationError(
      'INVALID_SOURCE_CONTEXT_AUTHORITY',
      'SourceContext must be SOURCE_CONTEXT authority'
    );
  }
  if (!sameAuthorityRef(sourceContext.semanticPayload.claimRef, claim.ref)) {
    throw new ScientificQualificationError(
      'CLAIM_SOURCE_CONTEXT_MISMATCH',
      'SourceContext does not bind the exact Claim'
    );
  }

  for (const name of ['sourceRef', 'sourceArtifactRef', 'compilationResultRef', 'sourceFaithfulReviewRef']) {
    if (!sameAuthorityRef(claim.semanticPayload[name], sourceContext.semanticPayload[name])) {
      throw new ScientificQualificationError(
        'CLAIM_SOURCE_CONTEXT_MISMATCH',
        `${name} differs between Claim and SourceContext`
      );
    }
  }
  if (claim.semanticPayload.sourceArtifactContentHash !== sourceContext.semanticPayload.sourceArtifactContentHash) {
    throw new ScientificQualificationError(
      'CLAIM_SOURCE_CONTEXT_MISMATCH',
      'SourceArtifact content hash differs between Claim and SourceContext'
    );
  }
  ...
  if (review.semanticPayload.disposition !== 'ACCEPT_SOURCE_FAITHFUL') {
    throw new ScientificQualificationError(
      'SOURCE_FAITHFUL_REVIEW_NOT_ACCEPTED',
      'Claim qualification requires ACCEPT_SOURCE_FAITHFUL review authority'
    );
  }

  const result = resolveKind(
    ledger,
    claim.semanticPayload.compilationResultRef,
    'ScientificCompilationResult',
    'COMPILATION_RESULT_REQUIRED'
  );
  const claimCandidate = resolveKind(
    ledger,
    review.semanticPayload.claimCandidateRef,
    'ClaimCandidate',
    'CLAIM_CANDIDATE_REQUIRED'
  );
  const sourceContextCandidate = resolveKind(
    ledger,
    review.semanticPayload.sourceContextCandidateRef,
  ...
  const artifact = resolveKind(
    ledger,
    claim.semanticPayload.sourceArtifactRef,
    'SourceArtifact',
    'SOURCE_ARTIFACT_AUTHORITY_REQUIRED'
  );
  const compilerDefinition = resolveKind(
    ledger,
    review.semanticPayload.compilerDefinitionRef,
  ...
  if (result.semanticPayload.outputAuthority !== 'PROPOSAL_ONLY'
    || compilerDefinition.semanticPayload.outputAuthority !== 'CANDIDATE_ONLY') {
    throw new ScientificQualificationError(
      'SOURCE_FAITHFUL_UPSTREAM_INVALID',
      'compiler/result authority must remain proposal-only/candidate-only'
    );
  }
  if (!includesExactRef(result.semanticPayload.claimCandidateRefs, claimCandidate.ref)
    ...
    throw new ScientificQualificationError(
      'SOURCE_FAITHFUL_UPSTREAM_INVALID',
      'review candidates are not members of the exact compilation result'
    );
  }
  if (!sameAuthorityRef(sourceContextCandidate.semanticPayload.claimCandidateRef, claimCandidate.ref)) {
    throw new ScientificQualificationError(
      'SOURCE_FAITHFUL_UPSTREAM_INVALID',
      'SourceContextCandidate does not belong to exact ClaimCandidate'
    );
  }
  if (!sameAuthorityRef(artifact.semanticPayload.sourceRef, source.ref)) {
    throw new ScientificQualificationError(
      'SOURCE_FAITHFUL_UPSTREAM_INVALID',
      'SourceArtifact does not bind exact Source'
    );
  }
  if (artifact.semanticPayload.contentHash !== claim.semanticPayload.sourceArtifactContentHash
    ...
    throw new ScientificQualificationError(
      'SOURCE_FAITHFUL_UPSTREAM_INVALID',
      'SourceArtifact content hash does not close through Claim/CompilationResult'
    );
  }
  if (!sameAuthorityRef(result.semanticPayload.sourceRef, source.ref)
    ...
    throw new ScientificQualificationError(
      'SOURCE_FAITHFUL_UPSTREAM_INVALID',
      'CompilationResult does not close to exact Source/Artifact/CompilerDefinition'
    );
  }
  if (!sameAuthorityRef(review.semanticPayload.sourceRef, source.ref)
    ...
    throw new ScientificQualificationError(
      'SOURCE_FAITHFUL_UPSTREAM_INVALID',
      'SourceFaithfulReviewDecision does not close to exact source chain'
    );
  }
  if (claim.semanticPayload.assertion !== claimCandidate.semanticPayload.assertion
    ...
    throw new ScientificQualificationError(
      'SOURCE_FAITHFUL_CLAIM_INVALID',
      'final Claim is not source-faithful to exact ClaimCandidate'
    );
  }
...
function assertQualificationAuthorization({
  ...
  if (!approverPrincipal || typeof approverPrincipal !== 'object' || Array.isArray(approverPrincipal)) {
    throw new ScientificQualificationError(
      'QUALIFICATION_PRINCIPAL_REQUIRED',
      'exact approverPrincipal is required'
    );
  }
  if (!audit?.actor
    || audit.actor.id !== approverPrincipal.principalId
    || audit.actor.type !== approverPrincipal.type) {
    throw new ScientificQualificationError(
      'QUALIFICATION_ACTOR_MISMATCH',
      'audit.actor must match exact scientific approver principal'
    );
  }
  ...
  if (storedDecision.operation !== 'KNOWLEDGE_QUALIFY' || storedDecision.allowed !== true) {
    throw new ScientificQualificationError(
      'QUALIFICATION_AUTHORIZATION_DENIED',
      'scientific qualification requires allowed KNOWLEDGE_QUALIFY authorization'
    );
  }
  if (!samePrincipalIdentity(storedDecision.principal, approverPrincipal)) {
    throw new ScientificQualificationError(
      'QUALIFICATION_PRINCIPAL_MISMATCH',
      'authorization principal differs from approverPrincipal'
    );
  }

  const policy = resolveKind(
    ledger,
    storedDecision.policyRef,
    'KnowledgeGovernancePolicy',
  ...
  if (policy.semanticPayload.resourceId !== qualificationResourceId(pair.claim.ref, pair.sourceContext.ref)) {
    throw new ScientificQualificationError(
      'QUALIFICATION_POLICY_RESOURCE_MISMATCH',
      'qualification policy is not bound to exact Claim + SourceContext authority'
    );
  }
  ...
  if (sourceOwnership.organizationId !== policyOwnership.organizationId
    || (sourceOwnership.tenantId ?? null) !== (policyOwnership.tenantId ?? null)) {
    throw new ScientificQualificationError(
      'QUALIFICATION_POLICY_OWNERSHIP_MISMATCH',
      'qualification policy ownership differs from Source ownership'
    );
  }
...
function auditEvent(base, suffix, inputRefs) {
  if (!base || typeof base !== 'object') {
    throw new ScientificQualificationError(
      'AUDIT_REQUIRED',
      'scientific qualification requires explicit audit metadata'
    );
  }
  return {
    ...base,
...
function normalizeDecisionPayload(record) {
  if (record.semanticPayload.authorityClass !== 'SCIENTIFIC_QUALIFICATION_DECISION') {
    throw new ScientificQualificationError(
      'INVALID_QUALIFICATION_DECISION',
      'ScientificQualificationDecision authorityClass is invalid'
    );
  }
  ...
  normalizeReasonCodes(record.semanticPayload.reasonCodes ?? [], {
    required: disposition === 'PROHIBIT_USE'
  });
  decisionSupersessionRefs(record);
  return deepFreeze({
    ...
    semanticPreconditions: canonicalObjectList(
      record.semanticPayload.semanticPreconditions ?? [],
      'semanticPreconditions'
    ),
    transportConstraints: canonicalObjectList(
      record.semanticPayload.transportConstraints ?? [],
...
function decisionsForPair(ledger, pair) {
  return ledger.exportSnapshot().records.filter((record) =>
...
function activeDecisionsForTarget(ledger, pair, target) {
  ...
  for (const decision of decisions) {
    for (const predecessorRef of decisionSupersessionRefs(decision)) {
      ...
      if (!sameAuthorityRef(predecessor.semanticPayload.claimRef, pair.claim.ref)
        ...
        throw new ScientificQualificationError(
          'INVALID_QUALIFICATION_SUPERSESSION',
          'qualification supersession crosses Claim/SourceContext/use target boundary'
        );
      }
...
function assertActiveDecisionForPublication(ledger, pair, decision) {
  ...
  if (!sameAuthorityRef(active[0].ref, decision.ref)) {
    throw new ScientificQualificationError(
      'STALE_QUALIFICATION_DECISION',
      `scientific use ${target.use} decision is superseded and cannot mint new QualifiedKnowledge`
    );
  }
}
...
function assertDecisionPublicationAudit(ledger, decision, authorization, pair) {
  ...
  const directEvents = ledger.auditFor(decision.ref)
    .filter((event) => sameAuthorityRef(event.objectRef, decision.ref));
  const requiredRefs = [
    pair.claim.ref,
  ...
  if (!valid) {
    throw new ScientificQualificationError(
      'QUALIFICATION_DECISION_AUDIT_INVALID',
      'qualification decision lacks direct approver audit binding exact Claim/SourceContext/authorization/policy'
    );
  }
}
...
function sortedDecisionRefs(records) {
  return deepFreeze([...records]
    .sort((left, right) =>
      targetKey(left.semanticPayload.qualificationTarget)
        .localeCompare(targetKey(right.semanticPayload.qualificationTarget)))
...
function aggregateConstraint(records, field) {
  ...
  for (const record of records) {
    ...
    for (const value of record.semanticPayload[field] ?? []) {
      entries.push({
        qualificationDecisionRef: record.ref,
        value: cloneCanonicalValue(value)
      });
    }
  }
  return deepFreeze(entries.sort((left, right) =>
    semanticHash('ADR-K04-Constraint', left)
      .localeCompare(semanticHash('ADR-K04-Constraint', right))));
}

function qualificationDecisionForTarget(ledger, qualifiedKnowledge, target) {
  const key = targetKey(target);
  for (const ref of qualifiedKnowledge.semanticPayload.qualificationDecisionRefs ?? []) {
    ...
    if (targetKey(decision.semanticPayload.qualificationTarget) === key) return decision;
  }
  return null;
}
...
export class ScientificQualificationService {
  ...
  constructor({ ledger }) {
    if (!ledger
      ...
      || typeof ledger.listVersions !== 'function'
      || typeof ledger.exportSnapshot !== 'function'
      || typeof ledger.auditFor !== 'function') {
      throw new ScientificQualificationError(
        'INVALID_LEDGER',
        'ScientificQualificationService requires shared replayable AuthorityLedger'
      );
    }
    this.#ledger = ledger;
  }
  ...
  recordQualificationDecision({
    ...
    if (!DISPOSITION_SET.has(normalizedDisposition)) {
      throw new ScientificQualificationError(
        'INVALID_QUALIFICATION_DISPOSITION',
        `unsupported disposition ${normalizedDisposition}`
      );
    }
    ...
    const normalizedReasons = normalizeReasonCodes(reasonCodes, {
      required: normalizedDisposition === 'PROHIBIT_USE'
    });
    const predecessorRefs = normalizeSupersessionInput({
      supersedesDecisionRef,
      supersedesDecisionRefs
    });

    const predecessorRecords = predecessorRefs.map((ref) => resolveKind(
      this.#ledger,
      ref,
      'ScientificQualificationDecision',
      'SUPERSEDED_QUALIFICATION_DECISION_REQUIRED'
    ));
    for (const predecessor of predecessorRecords) {
      if (!sameAuthorityRef(predecessor.semanticPayload.claimRef, pair.claim.ref)
        || !sameAuthorityRef(predecessor.semanticPayload.sourceContextRef, pair.sourceContext.ref)
        || targetKey(predecessor.semanticPayload.qualificationTarget) !== targetKey(target)) {
        throw new ScientificQualificationError(
          'INVALID_QUALIFICATION_SUPERSESSION',
          'superseded decision must concern the same Claim, SourceContext and qualification target'
        );
      }
    }
    ...
    if (!existingIdentity && predecessorRefs.length > 0) {
      const active = activeDecisionsForTarget(this.#ledger, pair, target);
      if (!refSetEquals(predecessorRefs, active.map((record) => record.ref))) {
        throw new ScientificQualificationError(
          'INCOMPLETE_QUALIFICATION_SUPERSESSION',
          `scientific use ${target.use} supersession must close the complete currently-active decision set`
        );
      }
    }
    ...
    const decision = this.#ledger.publish({
      ...
      semanticPayload: {
        ...
        supersedesDecisionRefs: predecessorRefs,
    ...
    for (const predecessor of predecessorRecords) {
      this.#ledger.addLineage({
        ...
        audit: auditEvent(audit, `decision-supersession-${exactRefKey(predecessor.ref)}`, [
          authorization.authAudit.ref
        ])
      });
    }
  ...
  publishQualifiedKnowledge({
    ...
    if (!Array.isArray(qualificationDecisionRefs) || qualificationDecisionRefs.length === 0) {
      throw new ScientificQualificationError(
        'QUALIFICATION_DECISIONS_REQUIRED',
        'QualifiedKnowledge requires at least one ScientificQualificationDecision'
      );
    }
    ...
    for (const decision of decisions) {
      ...
      if (!sameAuthorityRef(decision.semanticPayload.claimRef, pair.claim.ref)
        ...
        throw new ScientificQualificationError(
          'QUALIFICATION_DECISION_BUNDLE_MISMATCH',
          'all qualification decisions must bind the same exact Claim + SourceContext'
        );
      }
      if (!sameAuthorityRef(decision.semanticPayload.sourceRef, pair.source.ref)
        ...
        throw new ScientificQualificationError(
          'INVALID_QUALIFICATION_DECISION',
          'qualification decision does not bind the exact source-faithful authority chain'
        );
      }
      ...
      if (seenTargets.has(key)) {
        throw new ScientificQualificationError(
          'CONFLICTING_QUALIFICATION_DECISIONS',
          'a QualifiedKnowledge publication cannot contain multiple decisions for the same use target'
        );
      }
      ...
      const authorization = assertQualificationAuthorization({
        ...
        audit: {
          ...
          actor: {
            type: decisionPrincipal.type,
            id: decisionPrincipal.principalId
          }
        }
      });
      if (!sameAuthorityRef(authorization.policy.ref, decision.semanticPayload.qualificationPolicyRef)) {
        throw new ScientificQualificationError(
          'INVALID_QUALIFICATION_DECISION',
          'decision qualificationPolicyRef differs from exact authorization policy'
        );
      }
      assertDecisionPublicationAudit(this.#ledger, decision, authorization, pair);
      if (!existingIdentity) {
        assertActiveDecisionForPublication(this.#ledger, pair, decision);
      }
    ...
    if (allowedUses.length === 0) {
      throw new ScientificQualificationError(
        'QUALIFIED_USE_REQUIRED',
        'QualifiedKnowledge requires at least one QUALIFY_USE decision'
      );
    }

    const sortedAllowed = deepFreeze([...allowedUses]
      .sort((left, right) => targetKey(left).localeCompare(targetKey(right))));
    const sortedForbidden = deepFreeze([...forbiddenUses]
      .sort((left, right) => targetKey(left).localeCompare(targetKey(right))));

    let superseded;
    if (supersedesQualifiedKnowledgeRef) {
      superseded = resolveKind(
        this.#ledger,
        supersedesQualifiedKnowledgeRef,
        'QualifiedKnowledge',
        'SUPERSEDED_QUALIFIED_KNOWLEDGE_REQUIRED'
      );
      if (!sameAuthorityRef(superseded.semanticPayload.claimRef, pair.claim.ref)
        ...
        throw new ScientificQualificationError(
          'INVALID_QUALIFIED_KNOWLEDGE_SUPERSESSION',
          'requalification must preserve exact Claim + SourceContext identity'
        );
      }
    }
    ...
    if (superseded) {
      this.#ledger.addLineage({
        ...
        audit: auditEvent(
          audit,
          'qualified-knowledge-requalification',
          decisions.map((decision) => decision.ref)
        )
      });
    }
  ...
  revokeQualifiedKnowledgeUse({
    ...
    const qualifiedKnowledge = resolveKind(
      this.#ledger,
      qualifiedKnowledgeRef,
      'QualifiedKnowledge',
      'QUALIFIED_KNOWLEDGE_REQUIRED'
    );
    const target = normalizeScientificUseTarget(qualificationTarget);
    if (!(qualifiedKnowledge.semanticPayload.allowedUses ?? [])
      .some((allowed) => targetKey(allowed) === targetKey(target))) {
      throw new ScientificQualificationError(
        'QUALIFIED_USE_NOT_FOUND',
        'revocation target is not an allowed use of exact QualifiedKnowledge'
      );
    }
    if (this.qualifiedUseStatus({
      qualifiedKnowledgeRef: qualifiedKnowledge.ref,
      qualificationTarget: target
    }) === 'REVOKED') {
      throw new ScientificQualificationError(
        'QUALIFIED_USE_ALREADY_REVOKED',
        'exact QualifiedKnowledge use is already revoked'
      );
    }
    ...
    const selectedDecision = qualificationDecisionForTarget(this.#ledger, qualifiedKnowledge, target);
    if (!selectedDecision || selectedDecision.semanticPayload.disposition !== 'QUALIFY_USE') {
      throw new ScientificQualificationError(
        'QUALIFIED_USE_NOT_FOUND',
        'QualifiedKnowledge does not bind a QUALIFY_USE decision for revocation target'
      );
    }
    assertActiveDecisionForPublication(this.#ledger, pair, selectedDecision);

    const normalizedReasons = normalizeReasonCodes(reasonCodes, { required: true });
    const prohibitionDecision = this.recordQualificationDecision({
      decisionLogicalId: `${requiredText(revocationLogicalId, 'revocationLogicalId')}/prohibition`,
      decisionVersion: requiredText(revocationVersion, 'revocationVersion'),
      claimRef: pair.claim.ref,
      sourceContextRef: pair.sourceContext.ref,
      ...
      approverPrincipal,
      authorizationDecisionAuditRef,
      supersedesDecisionRefs: [selectedDecision.ref],
      audit: {
        ...audit,
        eventId: `${requiredText(audit?.eventId, 'audit.eventId')}:prohibition`
      }
    });
    ...
    const revocation = this.#ledger.publish({
      ...
      semanticPayload: {
        qualifiedKnowledgeRef: qualifiedKnowledge.ref,
        revokedQualificationDecisionRef: selectedDecision.ref,
        prohibitionDecisionRef: prohibitionDecision.ref,
        claimRef: pair.claim.ref,
        sourceContextRef: pair.sourceContext.ref,
        qualificationTarget: target,
        reasonCodes: normalizedReasons,
        ...(rationale ? { rationale: requiredText(rationale, 'rationale') } : {}),
        approverPrincipal: cloneCanonicalValue(approverPrincipal),
        authorizationDecisionAuditRef: prohibitionDecision.semanticPayload.authorizationDecisionAuditRef,
        qualificationPolicyRef: prohibitionDecision.semanticPayload.qualificationPolicyRef,
        authorityClass: 'SCIENTIFIC_USE_REVOCATION'
      },
      audit: auditEvent(audit, 'qualification-revocation', [
        qualifiedKnowledge.ref,
        selectedDecision.ref,
        prohibitionDecision.ref,
        pair.claim.ref,
        pair.sourceContext.ref,
        prohibitionDecision.semanticPayload.authorizationDecisionAuditRef,
        prohibitionDecision.semanticPayload.qualificationPolicyRef
      ])
    });
    ...
    this.#ledger.addLineage({
      ...
      audit: auditEvent(audit, 'qualification-revocation-lineage', [
        prohibitionDecision.ref
      ])
    });
  ...
  qualifiedUseStatus({ qualifiedKnowledgeRef, qualificationTarget }) {
    ...
    if ((qualifiedKnowledge.semanticPayload.forbiddenUses ?? [])
      .some((item) => targetKey(item) === key)) {
      return 'PROHIBITED';
    }
    if (!(qualifiedKnowledge.semanticPayload.allowedUses ?? [])
      .some((item) => targetKey(item) === key)) {
    ...
    for (const lineage of this.#ledger.lineageFor(qualifiedKnowledge.ref)) {
      ...
      const revocation = resolveKind(
        this.#ledger,
        lineage.from,
        'ScientificQualificationRevocation',
        'QUALIFICATION_REVOCATION_REQUIRED'
      );
