...
function timestamp(value, name) {
  ...
  if (month < 1 || month > 12
    ...
    throw new DeploymentError('INVALID_DEPLOYMENT_TIME', `${name} contains an impossible calendar date or clock time`);
  }
  if (zone !== 'Z') {
    const offsetHour = Number(zone.slice(1, 3));
    const offsetMinute = Number(zone.slice(4, 6));
    if (offsetHour > 14 || offsetMinute > 59 || (offsetHour === 14 && offsetMinute !== 0)) {
      throw new DeploymentError('INVALID_DEPLOYMENT_TIME', `${name} contains an invalid timezone offset`);
    }
  }
  const parsed = new Date(raw);
  if (Number.isNaN(parsed.getTime())) {
    throw new DeploymentError('INVALID_DEPLOYMENT_TIME', `${name} must be a valid RFC3339 timestamp`);
  }
