import { deepFreeze, semanticHash } from '../../canonicalization/src/index.mjs';
import { assertAuthorityRef } from '../../contracts/src/authority.mjs';

export const AGRONOMIC_CONTEXT_UNCERTAINTY_UNKNOWN_REPRESENTATION_CONTRACT_VERSION =
  'adr.agronomic-context-uncertainty-unknown-representation.v1';
export const AGRONOMIC_CONTEXT_UNCERTAINTY_UNKNOWN_REPRESENTATION_COMPILATION_CONTRACT_VERSION =
  'adr.agronomic-context-uncertainty-unknown-representation-compilation.v1';

export class AgronomicContextUncertaintyUnknownRepresentationCompilationError extends Error {
  constructor(code, message) {
    super(message);
    this.name = 'AgronomicContextUncertaintyUnknownRepresentationCompilationError';
    this.code = code;
  }
}

function requiredText(value, name) {
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new AgronomicContextUncertaintyUnknownRepresentationCompilationError(
      'INVALID_AGRONOMIC_CONTEXT_UNCERTAINTY_UNKNOWN_REPRESENTATION_INPUT',
      `${name} must be a non-empty string`
    );
  }
  return value.trim();
}

function exactObject(value, name, keys) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw new AgronomicContextUncertaintyUnknownRepresentationCompilationError(
      'INVALID_AGRONOMIC_CONTEXT_UNCERTAINTY_UNKNOWN_REPRESENTATION_INPUT',
      `${name} must be an object`
    );
  }
  for (const key of Object.keys(value)) {
    if (!keys.has(key)) {
      throw new AgronomicContextUncertaintyUnknownRepresentationCompilationError(
        'INVALID_AGRONOMIC_CONTEXT_UNCERTAINTY_UNKNOWN_REPRESENTATION_FIELD',
        `${name}.${key} is not part of the uncertainty unknown representation contract`
      );
    }
  }
}

function authorityRef(value, name, kinds) {
  let ref;
  try {
    ref = assertAuthorityRef(value);
  } catch (error) {
    throw new AgronomicContextUncertaintyUnknownRepresentationCompilationError(
      'INVALID_AGRONOMIC_CONTEXT_UNCERTAINTY_UNKNOWN_REPRESENTATION_AUTHORITY_REF',
      `${name} must be an exact authority ref: ${error?.message ?? 'invalid ref'}`
    );
  }
  if (!kinds.has(ref.kind)) {
    throw new AgronomicContextUncertaintyUnknownRepresentationCompilationError(
      'INVALID_AGRONOMIC_CONTEXT_UNCERTAINTY_UNKNOWN_REPRESENTATION_AUTHORITY_REF',
      `${name} must reference one of: ${[...kinds].join(', ')}`
    );
  }
  return ref;
}

function hashValue(value, name) {
  const normalized = requiredText(value, name);
  if (!/^sha256:[0-9a-f]{64}$/.test(normalized)) {
    throw new AgronomicContextUncertaintyUnknownRepresentationCompilationError(
      'INVALID_AGRONOMIC_CONTEXT_UNCERTAINTY_UNKNOWN_REPRESENTATION_HASH',
      `${name} must be a sha256 hash`
    );
  }
  return normalized;
}

function normalizeTargetContextSemantic(value) {
  exactObject(value, 'targetContextSemantic', new Set(['semanticId', 'value']));
  exactObject(value.value, 'targetContextSemantic.value', new Set(['type', 'date']));
  const semanticId = requiredText(value.semanticId, 'targetContextSemantic.semanticId');
  const type = requiredText(value.value.type, 'targetContextSemantic.value.type');
  const date = requiredText(value.value.date, 'targetContextSemantic.value.date');
  if (semanticId !== 'crop.planting_date' || type !== 'DATE' || date !== '2011-05-03') {
    throw new AgronomicContextUncertaintyUnknownRepresentationCompilationError(
      'UNSUPPORTED_AGRONOMIC_CONTEXT_UNCERTAINTY_UNKNOWN_REPRESENTATION_TARGET',
      'v1 supports only crop.planting_date = DATE 2011-05-03'
    );
  }
  return deepFreeze({
    semanticId,
    value: deepFreeze({ type, date })
  });
}

function normalizeUncertaintyRepresentation(value) {
  exactObject(value, 'uncertaintyRepresentation', new Set(['type', 'reasonCode']));
  const type = requiredText(value.type, 'uncertaintyRepresentation.type');
  const reasonCode = requiredText(
    value.reasonCode,
    'uncertaintyRepresentation.reasonCode'
  );
  if (
    type !== 'UNKNOWN'
    || reasonCode !== 'ACCEPTED_EVIDENCE_UNCERTAINTY_NOT_ESTABLISHED'
  ) {
    throw new AgronomicContextUncertaintyUnknownRepresentationCompilationError(
      'UNSUPPORTED_AGRONOMIC_CONTEXT_UNCERTAINTY_UNKNOWN_REPRESENTATION',
      'v1 supports only UNKNOWN with reasonCode=ACCEPTED_EVIDENCE_UNCERTAINTY_NOT_ESTABLISHED'
    );
  }
  return deepFreeze({ type, reasonCode });
}

function stringList(values, name) {
  if (!Array.isArray(values)) {
    throw new AgronomicContextUncertaintyUnknownRepresentationCompilationError(
      'INVALID_AGRONOMIC_CONTEXT_UNCERTAINTY_UNKNOWN_REPRESENTATION_INPUT',
      `${name} must be an array`
    );
  }
  const normalized = values.map((value, index) =>
    requiredText(value, `${name}[${index}]`)
  );
  if (new Set(normalized).size !== normalized.length) {
    throw new AgronomicContextUncertaintyUnknownRepresentationCompilationError(
      'DUPLICATE_AGRONOMIC_CONTEXT_UNCERTAINTY_UNKNOWN_REPRESENTATION_VALUE',
      `${name} cannot contain duplicates`
    );
  }
  return deepFreeze([...normalized].sort());
}

export function normalizeAgronomicContextUncertaintyUnknownRepresentation(value) {
  exactObject(
    value,
    'AgronomicContextUncertaintyUnknownRepresentation',
    new Set([
      'contractVersion',
      'representationId',
      'parentContextSemanticMappingCompilationRef',
      'targetContextSemantic',
      'uncertaintyRepresentation',
      'rationale'
    ])
  );
  if (requiredText(value.contractVersion, 'contractVersion')
      !== AGRONOMIC_CONTEXT_UNCERTAINTY_UNKNOWN_REPRESENTATION_CONTRACT_VERSION) {
    throw new AgronomicContextUncertaintyUnknownRepresentationCompilationError(
      'UNSUPPORTED_AGRONOMIC_CONTEXT_UNCERTAINTY_UNKNOWN_REPRESENTATION_CONTRACT',
      'unsupported uncertainty unknown representation contractVersion'
    );
  }
  return deepFreeze({
    contractVersion:
      AGRONOMIC_CONTEXT_UNCERTAINTY_UNKNOWN_REPRESENTATION_CONTRACT_VERSION,
    representationId: requiredText(value.representationId, 'representationId'),
    parentContextSemanticMappingCompilationRef: authorityRef(
      value.parentContextSemanticMappingCompilationRef,
      'parentContextSemanticMappingCompilationRef',
      new Set(['AgronomicRecordedOperationContextSemanticMappingCompilation'])
    ),
    targetContextSemantic: normalizeTargetContextSemantic(value.targetContextSemantic),
    uncertaintyRepresentation: normalizeUncertaintyRepresentation(value.uncertaintyRepresentation),
    rationale: requiredText(value.rationale, 'rationale')
  });
}

export function agronomicContextUncertaintyUnknownRepresentationHash(value) {
  return semanticHash(
    'AgronomicContextUncertaintyUnknownRepresentation',
    normalizeAgronomicContextUncertaintyUnknownRepresentation(value)
  );
}

export function normalizeAgronomicContextUncertaintyUnknownRepresentationCompilation(value) {
  exactObject(
    value,
    'AgronomicContextUncertaintyUnknownRepresentationCompilation',
    new Set([
      'contractVersion',
      'authorityClass',
      'representation',
      'representationHash',
      'uncertaintyRepresentationReviewRef',
      'losslessCoverage',
      'limitations'
    ])
  );
  if (requiredText(value.contractVersion, 'contractVersion')
      !== AGRONOMIC_CONTEXT_UNCERTAINTY_UNKNOWN_REPRESENTATION_COMPILATION_CONTRACT_VERSION) {
    throw new AgronomicContextUncertaintyUnknownRepresentationCompilationError(
      'UNSUPPORTED_AGRONOMIC_CONTEXT_UNCERTAINTY_UNKNOWN_REPRESENTATION_COMPILATION_CONTRACT',
      'unsupported uncertainty unknown representation compilation contractVersion'
    );
  }
  if (value.authorityClass
      !== 'AGRONOMIC_CONTEXT_UNCERTAINTY_UNKNOWN_REPRESENTATION_COMPILATION_AUTHORITY') {
    throw new AgronomicContextUncertaintyUnknownRepresentationCompilationError(
      'INVALID_AGRONOMIC_CONTEXT_UNCERTAINTY_UNKNOWN_REPRESENTATION_AUTHORITY',
      'invalid uncertainty unknown representation authorityClass'
    );
  }

  const representation =
    normalizeAgronomicContextUncertaintyUnknownRepresentation(value.representation);
  const representationHash = hashValue(value.representationHash, 'representationHash');
  if (representationHash
      !== agronomicContextUncertaintyUnknownRepresentationHash(representation)) {
    throw new AgronomicContextUncertaintyUnknownRepresentationCompilationError(
      'AGRONOMIC_CONTEXT_UNCERTAINTY_UNKNOWN_REPRESENTATION_HASH_MISMATCH',
      'representationHash must exactly match normalized representation'
    );
  }

  const uncertaintyRepresentationReviewRef = authorityRef(
    value.uncertaintyRepresentationReviewRef,
    'uncertaintyRepresentationReviewRef',
    new Set(['AgronomicContextUncertaintyUnknownRepresentationReviewDecision'])
  );

  exactObject(
    value.losslessCoverage,
    'losslessCoverage',
    new Set(['status', 'coveredElements', 'unrepresentedElements'])
  );
  const status = requiredText(value.losslessCoverage.status, 'losslessCoverage.status');
  if (!['COMPLETE', 'INCOMPLETE'].includes(status)) {
    throw new AgronomicContextUncertaintyUnknownRepresentationCompilationError(
      'INVALID_AGRONOMIC_CONTEXT_UNCERTAINTY_UNKNOWN_REPRESENTATION_COVERAGE',
      'losslessCoverage.status must be COMPLETE or INCOMPLETE'
    );
  }
  const coveredElements = stringList(
    value.losslessCoverage.coveredElements ?? [],
    'losslessCoverage.coveredElements'
  );
  const unrepresentedElements = stringList(
    value.losslessCoverage.unrepresentedElements ?? [],
    'losslessCoverage.unrepresentedElements'
  );
  if (status === 'COMPLETE' && unrepresentedElements.length !== 0) {
    throw new AgronomicContextUncertaintyUnknownRepresentationCompilationError(
      'INVALID_AGRONOMIC_CONTEXT_UNCERTAINTY_UNKNOWN_REPRESENTATION_COVERAGE',
      'COMPLETE uncertainty unknown representation cannot declare unrepresented targeted elements'
    );
  }
  if (status === 'INCOMPLETE' && unrepresentedElements.length === 0) {
    throw new AgronomicContextUncertaintyUnknownRepresentationCompilationError(
      'INVALID_AGRONOMIC_CONTEXT_UNCERTAINTY_UNKNOWN_REPRESENTATION_COVERAGE',
      'INCOMPLETE uncertainty unknown representation must name at least one unrepresented targeted element'
    );
  }

  return deepFreeze({
    contractVersion:
      AGRONOMIC_CONTEXT_UNCERTAINTY_UNKNOWN_REPRESENTATION_COMPILATION_CONTRACT_VERSION,
    authorityClass:
      'AGRONOMIC_CONTEXT_UNCERTAINTY_UNKNOWN_REPRESENTATION_COMPILATION_AUTHORITY',
    representation,
    representationHash,
    uncertaintyRepresentationReviewRef,
    losslessCoverage: deepFreeze({
      status,
      coveredElements,
      unrepresentedElements
    }),
    limitations: stringList(value.limitations ?? [], 'limitations')
  });
}

export function agronomicContextUncertaintyUnknownRepresentationCompilationAuthorityRefs(value) {
  const normalized =
    normalizeAgronomicContextUncertaintyUnknownRepresentationCompilation(value);
  return deepFreeze([
    normalized.representation.parentContextSemanticMappingCompilationRef,
    normalized.uncertaintyRepresentationReviewRef
  ]);
}
