...
export const AGRONOMIC_ACTION_REALIZATION_REVIEW_DISPOSITIONS = deepFreeze([
  'ACCEPT_ACTION_REALIZATION',
  'REJECT_ACTION_REALIZATION'
]);

const REVIEW_DISPOSITIONS = new Set(AGRONOMIC_ACTION_REALIZATION_REVIEW_DISPOSITIONS);
...
function requireLedger(ledger) {
  if (!ledger || typeof ledger.publish !== 'function'
    ...
    throw new AgronomicActionRealizationCompilationError(
      'INVALID_LEDGER',
      'AgronomicActionRealization authority requires a replayable AuthorityLedger'
    );
  }
}
...
function assertProtocolWorld({ ledger, sourceRefs, artifactRefs, world }) {
  for (const sourceRef of sourceRefs) {
    ...
    if (source.ref.kind !== 'Source' || source.semanticPayload?.sourceType !== 'PROTOCOL') {
      throw new AgronomicActionRealizationCompilationError(
        'AGRONOMIC_ACTION_REALIZATION_PROTOCOL_SOURCE_REQUIRED',
  ...
  for (const artifactRef of artifactRefs) {
    ...
    if (artifact.ref.kind !== 'SourceArtifact') {
      throw new AgronomicActionRealizationCompilationError(
        'AGRONOMIC_ACTION_REALIZATION_PROTOCOL_ARTIFACT_REQUIRED',
    ...
    if (!sourceRefs.some((sourceRef) => sameAuthorityRef(sourceRef, artifact.semanticPayload?.sourceRef))) {
      throw new AgronomicActionRealizationCompilationError(
        'AGRONOMIC_ACTION_REALIZATION_SOURCE_ARTIFACT_SOURCE_MISMATCH',
  ...
  if (!sameRefSet(sourceRefs, world.sources) || !sameRefSet(artifactRefs, world.artifacts)) {
    throw new AgronomicActionRealizationCompilationError(
      'AGRONOMIC_ACTION_REALIZATION_SCIENTIFIC_PREDECESSOR_MISMATCH',
      'Source/SourceArtifact sets must exactly equal active realization scientific predecessors'
    );
  }
}

function assertKnowledgeBindings(realization, knowledgeRefs) {
  const declared = new Set(knowledgeRefs.map(refKey));
  for (const binding of realization.authorityBindings) {
    if (!declared.has(refKey(binding.authorityRef))) {
      throw new AgronomicActionRealizationCompilationError(
        'AGRONOMIC_ACTION_REALIZATION_AUTHORITY_NOT_DECLARED',
        `action-realization authority binding ${binding.role} is not included in knowledgeRefs`
      );
    }
  }
}

function assertSourceExpressionEvidence(realization, world) {
  const matching = world.claims.some((claim) => {
    const assertion = claim.semanticPayload?.assertion;
    return typeof assertion === 'string' && assertion.includes(realization.sourceExpression);
  });
  if (!matching) {
    throw new AgronomicActionRealizationCompilationError(
      'AGRONOMIC_ACTION_REALIZATION_SOURCE_EXPRESSION_MISMATCH',
      'sourceExpression must occur in at least one exact source-qualified realization Claim predecessor'
    );
  }

  for (const alternative of realization.realizationSet.alternatives) {
    const expressions = alternative.kind === 'NAMED_METHOD'
      ? [alternative.sourceExpression]
      : [alternative.classExpression, alternative.membershipCriterionExpression];
    for (const expression of expressions) {
      if (!world.claims.some((claim) =>
        typeof claim.semanticPayload?.assertion === 'string'
        && claim.semanticPayload.assertion.includes(expression))) {
        throw new AgronomicActionRealizationCompilationError(
          'AGRONOMIC_ACTION_REALIZATION_ALTERNATIVE_SOURCE_MISMATCH',
          'every named/open-class realization expression must occur in exact source-qualified realization evidence'
        );
      }
    }
  }
...
function validateParentRegimen({ ledger, realization, world }) {
  const parent = validateAgronomicActionRegimenCompilationAuthority({
    ledger,
    compilationRef: realization.parentRegimenCompilationRef
  });
  const payload = parent.semanticPayload;
  if (payload.regimen.actionCode !== 'TILL' || realization.targetActionCode !== 'TILL') {
    throw new AgronomicActionRealizationCompilationError(
      'AGRONOMIC_ACTION_REALIZATION_PARENT_ACTION_MISMATCH',
      'DEC-0010 v1 requires exact parent regimen actionCode TILL'
    );
  }
  if (!sameRefSet(payload.sourceProtocolRefs, world.sources)
    || !sameRefSet(payload.sourceProtocolArtifactRefs, world.artifacts)) {
    throw new AgronomicActionRealizationCompilationError(
      'AGRONOMIC_ACTION_REALIZATION_PARENT_SOURCE_MISMATCH',
      'parent regimen and realization must close to the same exact Source/SourceArtifact world'
    );
  }
  return parent;
}

function validateRealizationWorld({ ledger, knowledgeRefs, realization, sourceRefs = null, artifactRefs = null }) {
  const world = validateKnowledgeWorld({ ledger, knowledgeRefs });
  assertKnowledgeBindings(realization, knowledgeRefs);
  assertSourceExpressionEvidence(realization, world);
  assertProtocolWorld({
    ledger,
  ...
  const parentRegimen = validateParentRegimen({ ledger, realization, world });
  return { world, parentRegimen };
}
...
function resolveAuthorizationForSource({ ledger, authRef, reviewerPrincipal, source }) {
  ...
  if (auth.ref.kind !== 'AuthorizationDecisionAudit') {
    throw new AgronomicActionRealizationCompilationError(
      'AGRONOMIC_ACTION_REALIZATION_REVIEW_AUTHORIZATION_REQUIRED',
      'semantic action-realization review requires AuthorizationDecisionAudit authority'
    );
  }
  ...
  if (decision.allowed !== true || decision.operation !== 'KNOWLEDGE_INSPECT'
    ...
    throw new AgronomicActionRealizationCompilationError(
      'AGRONOMIC_ACTION_REALIZATION_REVIEW_AUTHORIZATION_INVALID',
      'semantic action-realization review requires allowed KNOWLEDGE_INSPECT for the exact reviewer'
    );
  }
  ...
  if (!recomputed.allowed || recomputed.decisionHash !== decision.decisionHash) {
    throw new AgronomicActionRealizationCompilationError(
      'AGRONOMIC_ACTION_REALIZATION_REVIEW_AUTHORIZATION_INVALID',
      'semantic action-realization review authorization cannot be reproduced'
    );
  }
  ...
  if (!hasGrant) {
    throw new AgronomicActionRealizationCompilationError(
      'AGRONOMIC_ACTION_REALIZATION_REVIEWER_PERMISSION_DENIED',
...
function validateAuthorizationSet({ ledger, authorizationDecisionAuditRefs, reviewerPrincipal, sourceRefs }) {
  if (!Array.isArray(authorizationDecisionAuditRefs)
    ...
    throw new AgronomicActionRealizationCompilationError(
      'AGRONOMIC_ACTION_REALIZATION_REVIEW_AUTHORIZATION_SET_INVALID',
...
function assertAuditActor(audit, principal) {
  if (!audit?.actor || audit.actor.id !== principal.principalId || audit.actor.type !== principal.type) {
    throw new AgronomicActionRealizationCompilationError(
      'AGRONOMIC_ACTION_REALIZATION_REVIEW_ACTOR_MISMATCH',
      'audit actor must match the exact semantic review principal'
    );
  }
}

export function publishAgronomicActionRealizationReviewDecision({
  ledger,
  logicalId,
  version,
  knowledgeRefs,
  ...
  const validated = validateRealizationWorld({
    ledger,
  ...
  if (!Array.isArray(reasonCodes)) {
    throw new AgronomicActionRealizationCompilationError(
      'INVALID_AGRONOMIC_ACTION_REALIZATION_REVIEW_REASON',
      'reasonCodes must be an array'
    );
  }
  const normalizedReasons = [...new Set(reasonCodes.map((value) => text(value, 'reasonCode')))].sort();
  if (normalizedDisposition === 'REJECT_ACTION_REALIZATION' && normalizedReasons.length === 0) {
    throw new AgronomicActionRealizationCompilationError(
      'AGRONOMIC_ACTION_REALIZATION_REJECTION_REASON_REQUIRED',
      'rejected action-realization review requires at least one reason code'
    );
  }

  const realizationHash = agronomicActionRealizationHash(normalizedRealization);
  return ledger.publish({
    kind: 'AgronomicActionRealizationReviewDecision',
    logicalId: text(logicalId, 'logicalId'),
    version: text(version, 'version'),
    semanticPayload: {
      authorityClass: 'AGRONOMIC_ACTION_REALIZATION_SEMANTIC_REVIEW',
      knowledgeRefs: normalizedKnowledgeRefs,
      sourceRefs: validated.world.sources,
      sourceArtifactRefs: validated.world.artifacts,
      parentRegimenCompilationRef: validated.parentRegimen.record.ref,
      realization: cloneCanonicalValue(normalizedRealization),
      realizationHash,
    ...
    audit: {
      ...audit,
      action: 'REVIEW_AGRONOMIC_ACTION_REALIZATION',
      inputRefs: [
...
function validateReview({ ledger, reviewRef, normalized }) {
  ...
  if (review.ref.kind !== 'AgronomicActionRealizationReviewDecision'
    || review.semanticPayload?.authorityClass !== 'AGRONOMIC_ACTION_REALIZATION_SEMANTIC_REVIEW') {
    throw new AgronomicActionRealizationCompilationError(
      'AGRONOMIC_ACTION_REALIZATION_SEMANTIC_REVIEW_REQUIRED',
      'compilation requires exact AgronomicActionRealizationReviewDecision authority'
    );
  }
  const payload = review.semanticPayload;
  if (payload.disposition !== 'ACCEPT_ACTION_REALIZATION') {
    throw new AgronomicActionRealizationCompilationError(
      'AGRONOMIC_ACTION_REALIZATION_REVIEW_REJECTED',
      'only ACCEPT_ACTION_REALIZATION semantic review can authorize publication'
    );
  }
  if (payload.realizationHash !== normalized.realizationHash
    || agronomicActionRealizationHash(payload.realization) !== normalized.realizationHash
    ...
    || !sameAuthorityRef(payload.parentRegimenCompilationRef, normalized.realization.parentRegimenCompilationRef)) {
    throw new AgronomicActionRealizationCompilationError(
      'AGRONOMIC_ACTION_REALIZATION_REVIEW_MISMATCH',
      'semantic review must bind the exact realization, parent regimen and scientific predecessor closure'
    );
  }
  const authAudits = validateAuthorizationSet({
    ledger,
  ...
  const directAudit = ledger.auditFor(review.ref)
    ...
    .some((event) =>
      event.action === 'REVIEW_AGRONOMIC_ACTION_REALIZATION'
        ...
        && exactRefIn(event.inputRefs, normalized.realization.parentRegimenCompilationRef)
        && authAudits.every((item) => exactRefIn(event.inputRefs, item.ref))
    );
  if (!directAudit) {
    throw new AgronomicActionRealizationCompilationError(
      'AGRONOMIC_ACTION_REALIZATION_REVIEW_AUDIT_INVALID',
      'semantic action-realization review lacks direct reviewer audit over exact realization and parent-regimen predecessors'
    );
  }
  return review;
}
...
function validateCompilationWorld(ledger, normalized) {
  for (const ref of agronomicActionRealizationCompilationAuthorityRefs(normalized)) {
    const record = ledger.resolve(ref);
    if (!sameAuthorityRef(record.ref, ref)) {
      throw new AgronomicActionRealizationCompilationError(
        'AGRONOMIC_ACTION_REALIZATION_REF_MISMATCH',
        'every action-realization predecessor must resolve to its exact authority ref'
      );
    }
  }
  const validated = validateRealizationWorld({
...
export function publishAgronomicActionRealizationCompilation({ ledger, logicalId, version, compilation, audit }) {
  requireLedger(ledger);
  ...
  if (normalized.losslessCoverage.status !== 'COMPLETE') {
    throw new AgronomicActionRealizationCompilationError(
      ...
      'v1 action-realization authority may be published only with COMPLETE local realization coverage'
    );
  }
  ...
  const refs = agronomicActionRealizationCompilationAuthorityRefs(normalized);
  return ledger.publish({
    kind: 'AgronomicActionRealizationCompilation',
    logicalId: text(logicalId, 'logicalId'),
    version: text(version, 'version'),
    semanticPayload: cloneCanonicalValue(normalized),
    audit: {
      ...audit,
      action: 'PUBLISH_AGRONOMIC_ACTION_REALIZATION_COMPILATION',
      inputRefs: [...refs, ...(audit.inputRefs ?? [])],
      details: {
        ...(audit.details ?? {}),
        authorityClass: 'AGRONOMIC_ACTION_REALIZATION_COMPILATION_AUTHORITY',
        realizationHash: normalized.realizationHash,
...
export function validateAgronomicActionRealizationCompilationAuthority({ ledger, compilationRef }) {
  requireLedger(ledger);
  const record = ledger.resolve(compilationRef);
  if (record.ref.kind !== 'AgronomicActionRealizationCompilation') {
    throw new AgronomicActionRealizationCompilationError(
      'AGRONOMIC_ACTION_REALIZATION_COMPILATION_REQUIRED',
      `expected AgronomicActionRealizationCompilation, received ${record.ref.kind}`
    );
  }
  const normalized = normalizeAgronomicActionRealizationCompilation(record.semanticPayload);
  if (normalized.losslessCoverage.status !== 'COMPLETE') {
    throw new AgronomicActionRealizationCompilationError(
      'AGRONOMIC_ACTION_REALIZATION_INCOMPLETE_AUTHORITY_INVALID',
      'stored v1 action-realization authority must have COMPLETE local realization coverage'
    );
  }
  const world = validateCompilationWorld(ledger, normalized);
  const reviewer = world.review.semanticPayload.reviewerPrincipal;
  ...
  const directAudit = ledger.auditFor(record.ref)
    ...
    .some((event) =>
      event.action === 'PUBLISH_AGRONOMIC_ACTION_REALIZATION_COMPILATION'
        ...
        && sameAuthorityRef(event.details.parentRegimenCompilationRef, world.parentRegimen.record.ref)
    );
  if (!directAudit) {
    throw new AgronomicActionRealizationCompilationError(
      'AGRONOMIC_ACTION_REALIZATION_AUDIT_INVALID',
      'action-realization compilation lacks direct reviewer audit over all exact predecessors'
    );
  }
  return deepFreeze({
    record,
