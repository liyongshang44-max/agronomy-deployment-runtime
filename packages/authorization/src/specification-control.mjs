...
export const SPECIFICATION_RESOURCE_TYPES = deepFreeze([
...
export class SpecificationAuthorizationError extends Error {
  constructor(code, message) {
    super(message);
    this.name = 'SpecificationAuthorizationError';
    this.code = code;
  }
}
...
function text(value, name) {
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new SpecificationAuthorizationError('INVALID_SPECIFICATION_AUTHORIZATION_INPUT', `${name} must be a non-empty string`);
  }
  return value.trim();
}
...
export function authorizeSpecificationManage({ principal, roleAssignments, authorizationScope }) {
  const normalizedPrincipal = createPrincipal(principal);
  if (!Array.isArray(roleAssignments)) {
    throw new SpecificationAuthorizationError('INVALID_ROLE_ASSIGNMENTS', 'roleAssignments must be an array');
  }
  const resourceType = text(authorizationScope?.resourceType, 'authorizationScope.resourceType');
  if (!RESOURCE_TYPES.has(resourceType)) {
    throw new SpecificationAuthorizationError(
      'INVALID_SPECIFICATION_RESOURCE_TYPE',
      `unsupported specification resource type ${resourceType}`
    );
  }
  ...
  for (const candidate of roleAssignments) {
    if (!candidate?.ref || candidate.ref.kind !== 'RoleAssignment' || !candidate.semanticPayload?.principal) {
      throw new SpecificationAuthorizationError('INVALID_ROLE_ASSIGNMENT', 'expected published RoleAssignment record');
    }
    const payload = candidate.semanticPayload;
    if (!sameIdentity(payload.principal, normalizedPrincipal)) continue;
    if (!Array.isArray(payload.permissions) || !payload.permissions.includes(PERMISSIONS.SPECIFICATION_MANAGE)) continue;
  ...
  const reasons = [];
  if (!identityMatchesTarget) reasons.push('SPECIFICATION_TARGET_IDENTITY_DENIED');
  if (assignments.length === 0) reasons.push('SPECIFICATION_MANAGE_PERMISSION_DENIED');

  const payload = {
    operation: 'SPECIFICATION_MANAGE',
